...
BEGIN
    FOR ws IN SELECT id FROM workspaces LOOP
        ...
        -- Latest-attempt jobs only. skipped counts as finished. An active
        -- run whose latest jobs are all finished takes the normal roll-up:
        -- indeterminate, then failed, then canceled, otherwise succeeded.
        FOR run IN
            SELECT e.id
              FROM executions e
        ...
        LOOP
            SELECT
              ...
              INTO saw_indet, saw_failed, saw_canceled, saw_success, saw_skipped, n_jobs, n_open
            ...
            IF n_jobs IS NULL OR n_jobs = 0 OR n_open > 0 THEN
                CONTINUE;
            END IF;
            next_status := NULL;
            IF saw_indet THEN
        ...
        -- Parked runs on a workflow that was already soft-deleted. Same end
        -- state as delete: cancel waiting, pending, and blocked steps and
        -- jobs, fail the run, and close pending approvals as workflow_deleted.
        FOR run IN
            SELECT e.id
              FROM executions e
              JOIN workflows w
        ...
        -- Pending approvals whose latest gate step is no longer waiting.
        -- Match the step's end: workflow_deleted, run_canceled, or expired.
        WITH latest_gate AS (
            SELECT DISTINCT ON (s.execution_id, s.node_id)
                   s.execution_id,
                   s.node_id,
             ...
             ORDER BY s.execution_id, s.node_id, s.attempt DESC
        ),
        closed AS (
            UPDATE approvals a
               SET status = CASE
