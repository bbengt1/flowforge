...
type Catalog struct {
	...
	NBFLeeway      string        `json:"nbfLeeway"`
	MaxNBFLeeway   string        `json:"maxNbfLeeway"`
...
func fullCatalog(frames, issuers []string) Catalog {
	return Catalog{
		...
		NBFLeeway:    DefaultNBFLeeway.String(),
		MaxNBFLeeway: MaxNBFLeeway.String(),
		...
		KeyManagement: KeyManagement{
			...
			Env:           []string{EnvSigningKey, EnvSigningKeyFile, EnvSigningKeyID, EnvAudience, EnvAssertionTTL, EnvIssuer, EnvIssuerAllow, EnvOverlapKeys, EnvWebEmbedFrames, EnvExchangeRateLimitIP, EnvExchangeRateLimitPrincipal, EnvMintRateLimitPrincipal, EnvRateLimitWindow, EnvNBFLeeway, authz.EnvPlatformAdmins, authz.EnvPlatformAdmin},
			MaxOverlapTTL: MaxOverlapTTL.String(),
			Rotation:      "Mint with the durable active EMBED_SIGNING_KEY (PKCS#8 PEM via crypto/x509; production boot-fails if unset). Non-production may mint a crypto/rand ephemeral process key — never a committed seed. The active key is not an overlap key and does not use overlapUntil. JWKS publishes active + overlap after a store refresh. Every overlap verify key requires a short overlapUntil (max 4h). Missing, zero, or far-future is 400 on rotate or boot-fail on EMBED_OVERLAP_KEYS. Verify refreshes overlap and refuses missing/expired/retired/far-future kids. POST /embed/keys/rotate is platform-admin only and registers only the previous/current active public key as overlap. Unknown kid fails closed.",
		},
		Hooks: []HookStatus{
			...
			{ID: "exchange.rate-limit", Status: "ready", Fail: "burst POST /embed/exchange is 429 rate-limited", Note: "Keyed by client IP (default 120/min) and issuer|subject when claims are peekable (default 30/min). Window default 1m. Configurable via EMBED_EXCHANGE_RATE_LIMIT_IP, EMBED_EXCHANGE_RATE_LIMIT_PRINCIPAL, EMBED_RATE_LIMIT_WINDOW. Mint may use EMBED_MINT_RATE_LIMIT_PRINCIPAL (default 60/min). Soft-deny with 429 + problem detail. A nil limiter fails closed. Counters are shared in PostgreSQL when the API has a database so replica count does not multiply the budget; one process may keep an in-memory window. Login and machine token use separate keys."},
...
func claimDocs() []ClaimDoc {
	return []ClaimDoc{
		...
		{Name: "exp", Required: true, JSON: "exp", Note: "Unix seconds. Short TTL (default 60s, max 5m). No clock-skew leeway — expiry is exact."},
