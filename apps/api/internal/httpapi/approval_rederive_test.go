...
func TestDecidePersistsTargetedGateAfterDeny(t *testing.T) {
	h, admin, approvals := approvalRederiveServer(t)
	ws, tenant := currentWorkspace(t, h, admin)
	target := putMember(t, h, admin, tenant, ws, `{"issuer":"https://idp.example","external_subject":"rederive-target","role_keys":["approver"]}`)
	other := putMember(t, h, admin, tenant, ws, `{"issuer":"https://idp.example","external_subject":"rederive-other","role_keys":["approver"]}`)
	yaml := `apiVersion: flowforge/v1
kind: Workflow
metadata:
  name: rederive-user-gate
spec:
  triggers:
    - id: manual
      type: manual
  nodes:
    - id: gate
      ...
      with:
        ...
        approverUserId: ` + target.User.ID + `
        expiresIn: PT1H
  edges: []
`
	wf := createWorkflow(t, h, admin, tenant, ws, yaml)
	pub := publishWorkflow(t, h, admin, tenant, ws, wf.Workflow.ID, wf.Draft.Revision, "v1")
	exec := startExecution(t, h, admin, tenant, ws, wf.Workflow.ID, pub.Version.ID)
	rec := httptest.NewRecorder()
	req := workspaceJSON(http.MethodPost, "/api/v1/jobs/claim", []byte(`{"workerId":"rederive-target-worker","leaseSeconds":30}`), admin, tenant, ws)
	h.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("claim: %d %s", rec.Code, rec.Body.String())
	}
	...
	body, _ := json.Marshal(map[string]string{"decision": "approved"})
	rec = httptest.NewRecorder()
	req = workspaceJSON(http.MethodPost, "/api/v1/approvals/"+stale.ID+"/decide", body, other.User, tenant, ws)
	h.ServeHTTP(rec, req)
	...
	if denied.Status != approval.StatusPending || denied.DecidedBy != "" || denied.ApproverUserID != target.User.ID {
		t.Fatalf("after deny = %+v", denied)
	}
	rec = httptest.NewRecorder()
	req = workspaceRequest(http.MethodGet, "/api/v1/executions/"+exec.ID, nil, admin, tenant, ws)
	h.ServeHTTP(rec, req)
	var waiting wfstore.Execution
	if err := json.Unmarshal(rec.Body.Bytes(), &waiting); err != nil || waiting.Status != wfstore.ExecutionWaiting {
		t.Fatalf("run = %+v %v", waiting, err)
	}
	rec = httptest.NewRecorder()
	req = workspaceJSON(http.MethodPost, "/api/v1/approvals/"+stale.ID+"/decide", body, target.User, tenant, ws)
	h.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("target decide: %d %s", rec.Code, rec.Body.String())
	}
}
