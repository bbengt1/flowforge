package workflowhttp

import (
	"errors"
	"net/http"
	"strings"

	"github.com/bbengt1/flowforge/apps/api/internal/authz"
	"github.com/bbengt1/flowforge/apps/api/internal/httpapi/core"
	"github.com/bbengt1/flowforge/apps/api/internal/isolation"
	"github.com/bbengt1/flowforge/apps/api/internal/opsconfig"
	"github.com/bbengt1/flowforge/apps/api/internal/scripts"
	"github.com/bbengt1/flowforge/apps/api/internal/wfstore"
	"github.com/bbengt1/flowforge/apps/api/internal/workflow"
)

type revokeScriptRequest struct {
	ID             string `json:"id"`
	WorkspaceID    string `json:"workspace_id"`
	WorkspaceIDAlt string `json:"workspaceId"`
	Reason         string `json:"reason"`
}

type publishScriptRequest struct {
	ID                      string         `json:"id"`
	WorkspaceID             string         `json:"workspace_id"`
	WorkspaceIDAlt          string         `json:"workspaceId"`
	Language                string         `json:"language"`
	Source                  string         `json:"source"`
	Entrypoint              string         `json:"entrypoint"`
	RuntimeProfileID        string         `json:"runtimeProfileId"`
	RuntimeProfileVersionID string         `json:"runtimeProfileVersionId"`
	InputSchema             map[string]any `json:"inputSchema"`
	OutputSchema            map[string]any `json:"outputSchema"`
	TimeoutSeconds          int            `json:"timeoutSeconds"`
	MemoryMiB               int            `json:"memoryMiB"`
	CPUMillis               int            `json:"cpuMillis"`
	Processes               int            `json:"processes"`
}

func getScriptCatalog(s *core.Server, w http.ResponseWriter, r *http.Request) {
	scope, ok := opsScope(s, w, r, authz.PermOpsConfigView)
	if !ok {
		return
	}
	_ = scope
	core.WriteJSON(w, http.StatusOK, scripts.Catalog())
}

func publishScript(s *core.Server, w http.ResponseWriter, r *http.Request) {
	scope, ok := WorkflowScope(s, w, r, authz.PermWorkflowPublish)
	if !ok {
		return
	}
	if !requireScriptPipeline(s, w, r) {
		return
	}
	var req publishScriptRequest
	if !core.DecodeJSON(w, r, &req) {
		return
	}
	if core.HostIdentitySet(req.ID, req.WorkspaceID, req.WorkspaceIDAlt) {
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "Host-supplied workspace identity is not accepted.")
		return
	}
	pin, ok := resolveRuntimeProfilePin(s, w, r, scope, strings.TrimSpace(req.RuntimeProfileID), strings.TrimSpace(req.RuntimeProfileVersionID))
	if !ok {
		return
	}
	art, err := s.Scripts.Publish(r.Context(), scope, scripts.PublishInput{
		Language:                req.Language,
		Source:                  req.Source,
		Entrypoint:              req.Entrypoint,
		RuntimeProfileID:        pin.ResourceID,
		RuntimeProfileVersionID: pin.VersionID,
		RuntimeProfileDigest:    pin.Digest,
		RuntimeProfile:          pin.Spec,
		InputSchema:             req.InputSchema,
		OutputSchema:            req.OutputSchema,
		TimeoutSeconds:          req.TimeoutSeconds,
		MemoryMiB:               req.MemoryMiB,
		CPUMillis:               req.CPUMillis,
		Processes:               req.Processes,
	})
	if err != nil {
		writeScriptError(w, r, err)
		return
	}
	core.WriteJSON(w, http.StatusCreated, art)
}

func getScriptArtifact(s *core.Server, w http.ResponseWriter, r *http.Request) {
	scope, ok := WorkflowScope(s, w, r, authz.PermWorkflowView)
	if !ok {
		return
	}
	if !requireScriptPipeline(s, w, r) {
		return
	}
	id := strings.TrimSpace(r.PathValue("artifactId"))
	if id == "catalog" {
		w.Header().Set("Allow", "GET, HEAD")
		core.WriteProblem(w, r, http.StatusMethodNotAllowed, core.CodeMethodNotAllowed, "Method Not Allowed", "The "+r.Method+" method is not allowed for this path.")
		return
	}
	art, err := s.Scripts.Store.Get(r.Context(), scope, id)
	if err != nil {
		writeScriptError(w, r, err)
		return
	}
	core.WriteJSON(w, http.StatusOK, art)
}

func revokeScriptArtifact(s *core.Server, w http.ResponseWriter, r *http.Request) {
	scope, ok := WorkflowScope(s, w, r, authz.PermScriptRevoke)
	if !ok {
		return
	}
	if !requireScriptPipeline(s, w, r) {
		return
	}
	var req revokeScriptRequest
	if r.ContentLength > 0 {
		if !core.DecodeJSON(w, r, &req) {
			return
		}
		if core.HostIdentitySet(req.ID, req.WorkspaceID, req.WorkspaceIDAlt) {
			core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "Host-supplied workspace identity is not accepted.")
			return
		}
	}
	art, err := s.Scripts.Revoke(r.Context(), scope, scripts.RevokeInput{
		ArtifactID: strings.TrimSpace(r.PathValue("artifactId")),
		Reason:     req.Reason,
		Now:        Now(s),
	})
	if err != nil {
		writeScriptError(w, r, err)
		return
	}
	if s.Workflows != nil {
		_, _ = s.Workflows.WriteAudit(r.Context(), scope, wfstore.AuditWrite{
			Action:        scripts.AuditRevoke,
			ResourceType:  "script_artifact",
			ResourceID:    art.ID,
			Outcome:       "revoked",
			CorrelationID: core.RequestIDFromContext(r.Context()),
			Details:       scripts.RevokeAudit(art, scope.ActorID(), strings.TrimSpace(req.Reason)),
		})
	}
	core.WriteJSON(w, http.StatusOK, art)
}

func listWorkflowScriptArtifacts(s *core.Server, w http.ResponseWriter, r *http.Request) {
	scope, ok := WorkflowScope(s, w, r, authz.PermWorkflowView)
	if !ok {
		return
	}
	if !requireScriptPipeline(s, w, r) {
		return
	}
	workflowID := strings.TrimSpace(r.PathValue("workflowId"))
	versionID := strings.TrimSpace(r.PathValue("versionId"))
	if _, err := s.Workflows.GetVersion(r.Context(), scope, workflowID, versionID); err != nil {
		WriteWorkflowStoreError(w, r, err)
		return
	}
	items, err := s.Scripts.Store.ListVersionPins(r.Context(), scope, versionID)
	if err != nil {
		writeScriptError(w, r, err)
		return
	}
	core.WriteJSON(w, http.StatusOK, core.ListResponse[scripts.VersionPin]{Items: items})
}

func requireScriptPipeline(s *core.Server, w http.ResponseWriter, r *http.Request) bool {
	if s.Scripts != nil && s.Scripts.Store != nil {
		return true
	}
	core.WriteProblem(w, r, http.StatusServiceUnavailable, core.CodeDependencyUnavailable, "Dependency Unavailable", "Script artifact store is not available.")
	return false
}

func resolveRuntimeProfilePin(s *core.Server, w http.ResponseWriter, r *http.Request, scope isolation.Scope, resourceID, versionID string) (opsconfig.Pin, bool) {
	if s.Ops == nil {
		core.WriteProblem(w, r, http.StatusServiceUnavailable, core.CodeDependencyUnavailable, "Dependency Unavailable", "Operational configuration store is not available.")
		return opsconfig.Pin{}, false
	}
	if strings.TrimSpace(resourceID) == "" {
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "runtimeProfileId is required.")
		return opsconfig.Pin{}, false
	}
	pins, err := s.Ops.Resolve(r.Context(), scope, []opsconfig.Ref{{
		Kind:       opsconfig.KindRuntimeProfile,
		ResourceID: resourceID,
		VersionID:  versionID,
	}})
	if err != nil {
		WriteOpsError(w, r, err)
		return opsconfig.Pin{}, false
	}
	if len(pins) != 1 {
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "runtimeProfileId must resolve to a published revision.")
		return opsconfig.Pin{}, false
	}
	if err := scripts.ValidateRuntimeProfile("", pins[0].Spec); err != nil {
		writeScriptError(w, r, err)
		return opsconfig.Pin{}, false
	}
	return pins[0], true
}

func publishWorkflowScripts(s *core.Server, w http.ResponseWriter, r *http.Request, scope isolation.Scope, yamlDoc, versionID string, pins []opsconfig.Pin) ([]scripts.VersionPin, bool) {
	nodes := scriptNodeSpecs(yamlDoc)
	if len(nodes) == 0 {
		return []scripts.VersionPin{}, true
	}
	if !requireScriptPipeline(s, w, r) {
		return nil, false
	}
	var profiles []scripts.RuntimePin
	for _, pin := range pins {
		if pin.Kind == opsconfig.KindRuntimeProfile {
			profiles = append(profiles, scripts.RuntimePin{
				ResourceID: pin.ResourceID,
				VersionID:  pin.VersionID,
				Digest:     pin.Digest,
				Spec:       pin.Spec,
			})
		}
	}
	out, err := s.Scripts.PublishNodes(r.Context(), scope, versionID, nodes, profiles)
	if err != nil {
		if errors.Is(err, scripts.ErrImmutable) {
			existing, listErr := s.Scripts.Store.ListVersionPins(r.Context(), scope, versionID)
			if listErr != nil {
				writeScriptError(w, r, listErr)
				return nil, false
			}
			return existing, true
		}
		writeScriptError(w, r, err)
		return nil, false
	}
	return out, true
}

func VerifyWorkflowScriptPins(s *core.Server, w http.ResponseWriter, r *http.Request, scope isolation.Scope, yamlDoc, versionID string) bool {
	nodes := scriptNodeSpecs(yamlDoc)
	if len(nodes) == 0 {
		return true
	}
	if !requireScriptPipeline(s, w, r) {
		return false
	}
	if err := s.Scripts.VerifyNodePins(r.Context(), scope, versionID, nodes); err != nil {
		writeScriptError(w, r, err)
		return false
	}
	return true
}

func authorizeScriptNodes(s *core.Server, w http.ResponseWriter, r *http.Request, perms []string, yamlDoc string) bool {
	for _, node := range scriptNodeSpecs(yamlDoc) {
		_ = node
		for _, perm := range scripts.RequiredPermissions() {
			if !authz.Allows(perms, perm) {
				core.WriteForbidden(w, r)
				return false
			}
		}
	}
	return true
}

func scriptNodeSpecs(yamlDoc string) []scripts.NodeSpec {
	res, errs := workflow.ParseAndNormalize([]byte(yamlDoc))
	if len(errs) > 0 || res == nil || res.Document == nil {
		return nil
	}
	var out []scripts.NodeSpec
	for _, node := range res.Document.Spec.Nodes {
		if scripts.IsScriptNode(node.Type) {
			out = append(out, scripts.NodeSpec{ID: node.ID, Type: node.Type, With: node.With})
		}
	}
	return out
}

func scriptEngineError(err error) *scripts.EngineError {
	var ee *scripts.EngineError
	if errors.As(err, &ee) {
		return ee
	}
	return nil
}

func writeScriptError(w http.ResponseWriter, r *http.Request, err error) {
	var ee *scripts.EngineError
	if errors.As(err, &ee) && ee != nil {
		status := ee.Status
		if status == 0 {
			status = http.StatusBadRequest
		}
		code := ee.Code
		switch code {
		case scripts.CodeArtifactMutable:
			code = core.CodeArtifactMutable
		case scripts.CodeArtifactUnscanned:
			code = core.CodeArtifactUnscanned
		case scripts.CodeArtifactUnsigned:
			code = core.CodeArtifactUnsigned
		case scripts.CodeArtifactScanFailed:
			code = core.CodeArtifactScanFailed
		case scripts.CodeArtifactRevoked:
			code = core.CodeArtifactRevoked
		case scripts.CodeEmergencyStopped:
			code = core.CodeConflict
		case scripts.CodeEmergencyStopDenied, scripts.CodePolicyDenied:
			core.WriteForbidden(w, r)
			return
		case scripts.CodePermissionDenied:
			core.WriteForbidden(w, r)
			return
		case scripts.CodeIsolationDenied:
			code = core.CodeIsolationDenied
		case scripts.CodeMetadataDenied:
			code = core.CodeMetadataDenied
		case scripts.CodeEgressDenied:
			code = core.CodeEgressDenied
		case scripts.CodePackageInstallDenied:
			code = core.CodePackageInstallDenied
		case scripts.CodeImageDenied:
			code = core.CodeImageDenied
		case scripts.CodeResourceLimit:
			code = core.CodeResourceLimit
		case scripts.CodeIndeterminate:
			code = core.CodeIndeterminate
		case scripts.CodeRetryDenied:
			code = core.CodeRetryDenied
		case scripts.CodeHandleForbidden:
			code = core.CodeInvalidRequest
		case scripts.CodeEnvDenied:
			code = core.CodeInvalidRequest
		case scripts.CodeOutputTooLarge, scripts.CodeInputRejected, scripts.CodeInvalidVerification:
			code = core.CodeInvalidRequest
		default:
			code = core.CodeInvalidRequest
		}
		core.WriteProblem(w, r, status, code, http.StatusText(status), ee.Message)
		return
	}
	switch {
	case errors.Is(err, scripts.ErrNotFound):
		core.WriteProblem(w, r, http.StatusNotFound, core.CodeNotFound, "Not Found", "The requested resource was not found.")
	case errors.Is(err, scripts.ErrMutable):
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeArtifactMutable, "Invalid Request", "Mutable or draft script artifacts cannot be executed.")
	case errors.Is(err, scripts.ErrUnscanned):
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeArtifactUnscanned, "Invalid Request", "Unscanned script artifacts cannot be executed.")
	case errors.Is(err, scripts.ErrUnsigned):
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeArtifactUnsigned, "Invalid Request", "Unsigned script artifacts cannot be executed.")
	case errors.Is(err, scripts.ErrScanFailed):
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeArtifactScanFailed, "Invalid Request", "Script artifacts with a failed scan cannot be executed.")
	case errors.Is(err, scripts.ErrRevoked):
		core.WriteProblem(w, r, http.StatusConflict, core.CodeArtifactRevoked, "Conflict", "Revoked script artifacts cannot be executed.")
	case errors.Is(err, scripts.ErrImmutable), errors.Is(err, scripts.ErrConflict):
		core.WriteProblem(w, r, http.StatusConflict, core.CodeConflict, "Conflict", "Script artifacts and version pins are immutable.")
	case errors.Is(err, scripts.ErrStoreUnavailable):
		core.WriteProblem(w, r, http.StatusServiceUnavailable, core.CodeDependencyUnavailable, "Dependency Unavailable", "Script artifact store is not available.")
	case errors.Is(err, scripts.ErrInvalid), errors.Is(err, scripts.ErrNoScope), errors.Is(err, scripts.ErrSigningKey):
		core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "The script package is not valid.")
	default:
		core.WriteProblem(w, r, http.StatusInternalServerError, core.CodeInternalError, "Internal Server Error", "An unexpected error occurred.")
	}
}
