...
func getWorkflowCatalog(s *core.Server, w http.ResponseWriter, r *http.Request) {
	...
	core.WriteJSON(w, http.StatusOK, workflow.CoreCatalog())
}

func validateWorkflow(s *core.Server, w http.ResponseWriter, r *http.Request) {
	user, ok := s.RequirePrincipal(w, r)
	if !ok {
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, ValidateResponse{
		Valid:    true,
		Summary:  doc.Summary(),
...
func normalizeWorkflow(s *core.Server, w http.ResponseWriter, r *http.Request) {
	user, ok := s.RequirePrincipal(w, r)
	if !ok {
		return
	}
	...
	core.WriteJSON(w, http.StatusOK, NormalizeResponse{
...
func writeWorkflowErrors(w http.ResponseWriter, r *http.Request, errs workflow.ErrorList) {
	...
	core.WriteProblemErrors(w, r, http.StatusBadRequest, core.CodeInvalidWorkflow, "Invalid Workflow", detail, out)
}
	...
	case "application/yaml", "application/x-yaml", "text/yaml", "text/x-yaml":
		...
		if err != nil {
			...
			if errors.As(err, &maxBytes) {
				core.WriteProblem(w, r, http.StatusRequestEntityTooLarge, core.CodeRequestTooLarge, "Request Too Large", "The request body exceeds the 1048576 byte limit.")
				return nil, false
			}
			core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "Request body could not be read.")
			return nil, false
		}
		if len(strings.TrimSpace(string(body))) == 0 {
			core.WriteProblem(w, r, http.StatusBadRequest, core.CodeInvalidRequest, "Invalid Request", "Request body is required.")
			return nil, false
		}
		return body, true
	default:
