...
func triggerTypes() []TriggerType {
	...
	return []TriggerType{
		...
		},
		{
			...
			Phase:       PhaseCore,
			Title:       "Schedule",
			Description: "Timezone-explicit cron or interval start pinned to a published workflow version. Safe defaults: skip overlap, ignore misfires, catchUp=0.",
			Outputs:     event,
			AllowedWith: []WithField{
				{Name: "timezone", Kind: "string", Required: true, Description: "IANA timezone. Required; no implicit local TZ."},
				{Name: "cron", Kind: "string", Description: "5-field cron. Exactly one of cron or interval."},
				{Name: "interval", Kind: "duration", Description: "ISO-8601 interval. Exactly one of cron or interval."},
				{Name: "overlapPolicy", Kind: "enum", Required: true, Enum: []string{"skip", "reject", "queue"}, Description: "Default skip: one active execution unless the workflow is verified idempotent."},
				{Name: "misfirePolicy", Kind: "enum", Enum: []string{"ignore", "fire-once"}, Description: "Default ignore: do not replay missed slots."},
				{Name: "catchUp", Kind: "integer", Description: "Missed-slot replay bound 0–5. Default 0."},
			},
			Bounds: defaultNeutralBounds(),
			Redaction: &RedactionPolicy{
				AuditFields:   []string{"scheduleId", "workflowVersionId", "workflowDigest", "correlationId", "outcome", "idempotencyKey", "timezone"},
				RedactInputs:  true,
				RedactOutputs: true,
				Strategy:      "mask-classified",
			},
			Admin: &TriggerAdmin{
				...
				CSRF:                true,
				SecretNeverReturned: false,
				Help:                "Cookie session + X-CSRF-Token. Create pins a published workflowVersionId. Timezone is a required IANA name. Safe defaults: overlapPolicy=skip, misfirePolicy=ignore, catchUp=0. Dispatcher starts with E10.1 idempotency/authz/policy and fails closed when the schedule or published version is disabled. Host-supplied id/workspaceId is 400.",
			},
		},
