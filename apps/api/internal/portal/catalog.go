...
func adapterAPI() []APIRoute {
	return []APIRoute{
		...
		{Method: "POST", Path: "/api/v1/embed/exchange", Auth: "assertion", CSRF: "no", Note: "E11.1/E11.2 exchange. Not a Portal-specific path. Signature/claims verify before any workspace lookup. Mint writes ctx=portal and host=iss; exchange selects PORTAL_* from that signed claim. Optional X-FlowForge-Host-Issuer / hostContext=portal must agree. Replay 409. Binds (tenant_id, workbench_key) onto CHIPS cookies (SameSite=None; Secure; Partitioned). Bound sessions cannot create tenants or sibling workbenches. Cookie not sent is 401/403."},
		{Method: "GET", Path: "/api/v1/embed/catalog", Auth: "none (disclosure follows peeked ff_session)", CSRF: "no", Note: "Embed SDK/contract. Portal adapter builds on this. Membership/isolation routes are omitted unless the session grants workspace.administer or platform.administer (ADV-024). frameAncestors and issuers stay published."},
