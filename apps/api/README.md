...
Go module `github.com/bbengt1/flowforge/apps/api` (Go **1.26**). Listens on **8080** and exposes the routes from `docs/reference/backend-api-map.md`. Operator OpenAPI guide: [`docs/reference/openapi.md`](../../docs/reference/openapi.md) (`openapi/openapi.yaml` is generated from the route table; do not hand-edit it). Release/ops: [`docs/operations/index.md`](../../docs/operations/index.md).
...
| `TLS_CERT_FILE` / `TLS_KEY_FILE` | empty (compose: `/tmp/flowforge-tls/{cert,key}.pem`) | Optional process TLS. Both must be set or neither. B.5 writes created/uploaded PEMs here (0600). Parent dir is created (0700) on write. Empty fails closed on create/upload. Skip (B.7) does not require the paths. Compose defaults writable `/tmp` paths for path-2. Do not copy to k8s. |
...
| `QUOTA_MUTATE_BURST` | per-minute rate | Mutation token-bucket capacity. |
| `QUOTA_READ_PER_MINUTE` | `300` | Per-workspace refill for lists and other expensive reads. |
| `QUOTA_READ_BURST` | per-minute rate | Read token-bucket capacity. |
| `QUOTA_DOWNLOAD_PER_MINUTE` | `60` | Per-workspace artifact download refill. |
