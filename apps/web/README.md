...
`/membership` is the E2.1 operator UI (Chloe). `/isolation` is the E2.2 negative isolation exercise (also embedded on `/membership`). Both proxy jonny's `/api/v1` routes under `/api/control-plane/*` with FlowForge headers and `X-Request-ID`. Dev identity is tab-scoped `sessionStorage` until E2.3 sessions; workspace lookup is tenant + workbench key, never a host-supplied workspace UUID.
