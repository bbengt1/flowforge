  ...
  return (
    ...
    >
      ...
      {view.workflow.length > 0 ? (
        <div className="mt-4">
          <h3 className="text-sm font-medium text-fg">Workflow notes</h3>
      ...
      <div className="mt-4 space-y-1 font-mono text-xs text-fg">
      ...
      <div className="mt-4 space-y-2 text-sm" data-ndv-evaluate>
        {evaluationPending ? (
          <p className="text-fg">Evaluating published-version policy…</p>
        ) : null}
        {evaluationProblem ? (
          <p className="text-fg">
        ...
        {!scoped.publishedVersionInPlay ? (
          <p className="text-fg">{NDV_NO_DRAFT_EXECUTE_HELP}</p>
        ) : null}
        ...
        {scoped.workflowNotes
          ...
          .map((note) => (
            <p key={note} className="text-xs text-fg">
              {note}
            </p>
          ))}
        {approvals.length > 0 ? (
          <div className="space-y-2">
            <p className="text-xs text-fg">{NDV_APPROVAL_DECIDE_HELP}</p>
            {approvals.map((item) => (
              <p key={item.id}>
                <Link
                ...
                {item.status ? (
                  <span className="text-xs text-fg"> · {item.status}</span>
                ) : null}
              </p>
            ))}
          </div>
