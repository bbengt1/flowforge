  ...
  return (
    ...
    >
      ...
      <div className="mt-4 rounded-xl border border-border bg-background p-3">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{webhookIngressHelp(catalog)}</p>
        <p className="mt-2 text-xs text-muted-foreground">{WEBHOOK_INGRESS_HELP}</p>
      </div>
                ...
                {canManage ? (
                  <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]">
                    ...
                    <button
                      ...
                      className="self-end rounded-lg border border-border bg-card px-3 py-1.5 text-sm hover:bg-background disabled:opacity-60"
                    >
      ...
      {canManage ? (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">{WEBHOOK_FIELD_MAPPING_HELP}</p>
          {yaml ? (
            <button
              type="button"
              onClick={() =>
          ...
          {!editingId ? (
            <>
              <fieldset className="grid gap-2">
                <legend className="text-sm text-muted-foreground">Secret</legend>
                <Field
            ...
            <p className="text-xs text-muted-foreground">{WEBHOOK_ROTATE_SECRET_HELP}</p>
          )}
          <Field
          ...
          >
            <textarea
              ...
              className="mt-1 w-full rounded-lg border border-border bg-card px-3 py-1.5 font-mono text-xs"
            />
          </Field>
          ...
          {localErrors.length > 0 ? (
            <FieldError id="webhook-form-error" className="text-sm text-destructive">
        ...
        <p className="mt-4 text-sm text-muted-foreground">{WEBHOOK_FORBIDDEN_MESSAGE}</p>
      )}
      {deleting ? (
        <ConfirmDestructive
          open
...
function CopyButton({ label, value }: { label: string; value: string }) {
  ...
  return (
    <button
      ...
      className="rounded-lg border border-border bg-card px-2 py-1 text-xs hover:bg-background"
    >
      {copied ? "Copied" : label}
    </button>
  );
