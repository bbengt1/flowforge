            ...
            {placeable.map((node) => (
              <li key={node.id}>
                ...
                >
                  ...
                  <span className={`mx-1 ${node.id === selectedId ? "text-teal-800" : "text-muted-foreground"}`}>
                    ·
                  </span>
                  {node.name || node.type}
                </button>
...
function SelectedNodeIdentity({
  ...
  return (
    <div className="mt-4 space-y-3">
      <p className="font-mono text-xs text-muted-foreground">{node.type}</p>
...
function NodeConfigForm({
  ...
  return (
    ...
    >
      ...
      {errors.length > 0 ? (
        <FieldError id="ndv-node-errors" className="space-y-1 text-sm text-warning-foreground">
          {errors.map((error) => (
      ...
      return (
        <>
          ...
          {config.op !== "exists" ? (
            ...
            <p className="text-xs text-muted-foreground">exists must not include compare.</p>
          )}
        </>
      );
    case "flow.delay":
...
function SetFieldsEditor({
  ...
  return (
    <fieldset className="space-y-2" data-ndv-field="fields">
      <legend className="text-sm text-muted-foreground">value fields</legend>
        ...
        <div key={`${field.key}-${index}`} className="space-y-1 rounded-lg border border-border p-2">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className="min-w-0 flex-1 rounded-lg border border-border px-2 py-1.5 font-mono text-xs disabled:bg-background"
            />
          </div>
          <button
            type="button"
      ...
      <button
        ...
        className="text-sm text-accent-text underline decoration-teal-200 underline-offset-2"
      >
        Add field
      </button>
    </fieldset>
...
function MapFieldsEditor({
  ...
  return (
    <fieldset className="space-y-2" data-ndv-field-path-editor="core-map" data-ndv-field="mapping">
      <legend className="text-sm text-muted-foreground">mapping dest → from</legend>
      <p className="text-xs text-muted-foreground">
