...
export function ExecutionApprovalState({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-muted-foreground">{APPROVAL_BINDING_HELP}</p>
      ...
      {waitControls.waitEnabled ? (
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button
            ...
            className="rounded-lg border border-border bg-background px-3 py-1.5 text-sm text-foreground disabled:cursor-not-allowed disabled:opacity-60"
          >
            Wait for approval
          </button>
          <button
      ...
      <p className="mt-2 text-xs text-muted-foreground">{waitControls.resumeHelp}</p>
    </section>
  );
}
