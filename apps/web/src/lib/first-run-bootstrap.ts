/**
 * B.6: First-run wizard chrome + Settings handoff.
 *
 * Relates to #339 / Part of #333. Keep #339 open.
 *
 * Chloe UI only. Consumes jonny's B.1–B.5 status-only APIs
 ...
 *
 * After complete the wizard never remounts. URL / TLS / users /
 * persistence edits live in Settings and link out.
 */
...
export const FIRST_RUN_BOOTSTRAP_HELP =
  "Standalone first-run wizard (persistence → first admin → public URL → TLS). Incomplete GET shows the wizard; complete or 401 goes to product home. Never on /embed/v1. After complete, Settings links out — the wizard does not remount. Passwords, PEMs, and KEK are never stored in the browser.";
...
export const BOOTSTRAP_STEP_HELP = {
  ...
  tls:
    "Enable TLS by creating a self-signed certificate or uploading a PEM pair once. ACME / Let’s Encrypt is out of scope. Private keys POST once and are never stored in the browser.",
} as const satisfies Record<BootstrapStepId, string>;
