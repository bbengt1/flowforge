...
describe("resolveIdentityProxyTarget", () => {
  it("maps the documented E2.1, E2.2, E2.3, E3.1, and E3.2 routes onto /api/v1", () => {
    const cases: Array<[string, string[], string]> = [
      ...
      ["GET", ["portal", "adapter"], "/api/v1/portal/adapter"],
      ["POST", ["portal", "adapter", "assertions"], "/api/v1/portal/adapter/assertions"],
  ...
  it("rejects unknown paths and disallowed methods with problem codes", () => {
    ...
    const getPersistence = resolveIdentityProxyTarget("GET", [
      "bootstrap",
      "persistence",
    ]);
    ...
    const getTLS = resolveIdentityProxyTarget("GET", ["bootstrap", "tls"]);
    assert.equal("status" in getTLS, true);
    if ("status" in getTLS) {
      assert.equal(getTLS.status, 405);
    }
