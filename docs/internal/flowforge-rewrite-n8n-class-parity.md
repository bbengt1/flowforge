...
| Membership / isolation | ADV-024 | Keep grant-gated; stop looking like the product | **Reshape** (R7.2 / #277 — **keep #277 open**): copy/placement off product chrome; Settings may link carefully; **do not** retire the grant |
...
| Membership / isolation | ADV-024 grant; exercise UX | **Reshape** (R7.2 / #277 — **keep #277 open**) | Keep fail-closed grant and negative tests; retire “this is the product” framing. Isolation success is a denial. |
| Local seed (#191) | `local` / `default` + demo vault in non-prod | **Keep** (R7.3 / #278 — **keep #278 open**) | Labeled **Example context** stays local-only. Trusted-dev header fallback is never rewrite login. Production boot-fail if forced on. Rewrite must not copy seed into k8s. |
...
| Local seed | Keep local-only; demo credentials stay placeholders. R7.3 (#278 — **keep #278 open**) keeps Example context labeled and does not promote trusted-dev as rewrite login. |
...
- A11y: R7.4 (#279 — **Keep #279 open.**) extends UX.10 to NDV, palette, Runs overlay, activation chrome, and other R2–R6 satellites; still no SR graph rewrite unless Brent files it.
...
5. **A11y** — R7.4 (#279 / Part of #233 — **Keep #279 open.**) extends UX.10 (Esc, focus return, no nested `<main>`, icon+text) to NDV, palette, Runs overlay, activation chrome, and other R2–R6 satellites. Still no screen-reader graph rewrite unless Brent files it.
...
| Folders / projects as resources | **Promoted** (Brent) — nested folder resource, not tags-first | **Post-R7** ([folder IA](flowforge-workflow-folders.md)) | Home folder filters are a **client-side** name/slug prefix today. First-class: server-backed `folderId` on `workflows` (not a second tenancy axis). Do not bypass workspace RLS. Tags remain out of that epic. |
