...
The same backends are refused when the process is production-locked, including a single replica. Empty, `production`, and unknown `APP_ENV`, and `REQUIRE_TLS=true`, are production-locked. The operator error names the in-memory domains (workflows, vault, executions, sessions, and the other process-local stores) and tells you to set `DATABASE_URL` plus an S3 artifact store. There is no flag that allows memory in production. `APP_ENV=development|dev|local|test` with `REQUIRE_TLS` off may still compose in-memory stores for unit tests and local development. Compose keeps Postgres; the memory path is the non-pool test and dev composition, not a production fallback.
