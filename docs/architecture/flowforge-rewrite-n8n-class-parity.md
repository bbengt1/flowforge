...
1. **Discoverability.** YAML and runs stay hidden on first paint. R2.1 makes the palette a remembered-open satellite; R2.2 (#235) does the same for the NDV-style inspector shell so both are *available without hunting*. Typed mapping depth still follows in R3. FlowForge uses its own persistent/satellite layout — it does not need n8n’s exact columns.
...
3. **Missing graph primitives.** Undo/redo for canvas graph edits landed in R2.3 (#236). Fit-to-workflow, snap-to-grid, minimap, multi-select, and alignment remain aspirational in [frontend-ui](../reference/frontend-ui.md). n8n-class authoring assumes most of these.
...
- Graph primitives (phase R2): undo/redo landed (R2.3 / #236); multi-select, fit, snap still follow; minimap/alignment if they do not steal viewport. Keyboard remains the MVP path.
...
| **R6 — Activations and triggers** | Home + editor expose “this published version is active” for webhook/schedule (and manual start) without teaching three drawers | Trigger admin APIs already on main. [D2](#d2--activation-model) locked: compose existing enable + version pin; no new activation resource. Drafts still never run | Chloe; jonny compose/read-model only | R1 D2, E10 on main |
...
| **Editor / canvas** | `/workflows/{id}` viewport; auto-layout only; invalid YAML never guesses a graph; undo/redo for graph edits (R2.3 / #236) | Remaining graph primitives (multi-select, fit/snap). Layout persist via [D1](#d1--canvas-layout-persistence) `metadata.ui.layout` (API landed; Chloe #238). Touch stays a breakpoint, not a mobile app. |
...
| **NDV / inspector** | Right rail, remembered-open satellite (R2.2 / #235): parameters / `with` / pins / display-name credentials; workflow tabs Triggers / Versions / Pins; redacted last-run I/O | Typed field-path mapping depth (R3). No expression language. No `SecretField` in the rail. No branded NDV modal. |
