...
Status: **charter frozen (D1–D6 locked)**. This document does not change application code, contracts, or shipped behavior. It is the successor program after E1–E12. R1–R7 are the rewrite phases. Do **not** treat it as a replacement of the [master implementation plan](../master-implementation-plan.md) for shipped E1–E12 work.
...
Epic (UX.1–UX.12) already moved `/workflows/{id}` to canvas-first chrome on `main`. That is the **current baseline to migrate from**, not the end state. The canvas is now the primary surface, but the interaction model is still a FlowForge operator shell with drawers you must discover: hidden-on-first-paint library/YAML/runs, auto-layout-only graph, form-rail inspector rather than a node-detail conversation, and “publish + start published + trigger admin” instead of a single activation mental model.

**Decision already taken (do not reopen unless explicitly reverses it):** parity of interaction and coverage; independence of implementation, visual language, and legal identity.
...
- **Preserve security and product invariants** unless this document (or a later decision recorded here) replaces one with a safer equivalent and a migration.
- **Epic-ready phases** so the phased plan can be turned into stories from [§9](#9-phased-rewrite-plan) without inventing scope.
- **Fold-in without restructure.** Paste drafts into [§11](#11-fold-in-stubs) only.
...
- Putting trigger types on the canvas as graph nodes ([D3](#d3--trigger-placement) locked: workflow-level). Revisit only as an explicit reversal with a YAML migration.
...
Shipped on `main` at the time of this charter: **E1–E12** (platform through production readiness) plus **ADV** (epic family) plus **epic ** canvas-first chrome (UX.1–UX.12, including the UX.12 doc/walkthrough update). Normative current-state docs:
...
- Epic landed: `/workflows` product home; `/workflows/{id}` viewport canvas + sticky top bar; library / YAML / runs as drawers; inspector node/`with`/pins/credentials + workflow tabs Triggers / Versions / Pins; last-run overlay on the **same** canvas; nav collapses to an icon-rail on the editor.

### What still feels wrong versus n8n-class UX

These are product gaps, not invitations to clone n8n chrome.

1. **Discoverability.** YAML stays hidden on first paint. R2.1 makes the palette a remembered-open satellite; R2.2 does the same for the NDV-style inspector shell; R4.2 does the same for the Runs overlay so those surfaces are *available without hunting*. Typed mapping depth still follows in R3. FlowForge uses its own persistent/satellite layout — it does not need n8n’s exact columns.
...
3. **Missing graph primitives.** Undo/redo for canvas graph edits landed in R2.3. Multi-select, fit-to-view, and snap-to-grid landed in R2.4. Minimap and alignment remain aspirational in [frontend-ui](../reference/frontend-ui.md). n8n-class authoring assumes most of these.
...
7. **Foundation surfaces leak.** Membership/isolation stay grant-gated (correct). R7.2 takes them off product chrome; Settings may still *link* carefully. R7.3 keeps labeled **Example context** local-only and does not promote trusted-dev header fallback as rewrite login. Developer fixtures stay disclosed under Settings.
...
9. **No first-class “try this node”** that stays inside publish-then-run. n8n’s execute-unsaved is **not** the target. The missing FlowForge move is a one-gesture **publish test version → run** (or a -approved safer equivalent).
10. **Narrow/touch is a breakpoint**, not an inspector-first editor (`touch-inspector-first` in frontend-ui).
11. **Control-plane density gaps** (not blocking E6): no execution-vs-execution compare route; no replay projection endpoint; client stitches version YAML + steps. Catalog metadata is complete for core/K8s/SSH/script/HTTP on `main`; UI still has contract-fallback paths. Fold-in and R# marks: [§11.2](#112-control-plane-execution-and-credential-parity-gaps).
...
- Palette available without a scavenger hunt (persistent satellite or one-click that stays open across node adds — the FlowForge pattern is specified in [§11](#111-ui-surfaces-and-operator-migration-notes)).
...
- Spatial layout: see [D1](#d1--canvas-layout-persistence). Persist optional non-authoritative `metadata.ui.layout` (API stores/returns; executor ignores). Invalid or missing layout → auto-layout. Never invent nodes or edges. Never a second canvas file. The canvas wires the canvas on.
- Graph primitives (phase R2): undo/redo landed (R2.3); multi-select, fit, snap landed (R2.4); minimap/alignment if they do not steal viewport. Keyboard remains the MVP path.

### Node palette

Enabled catalog only (`GET /workflows/catalog` plus engine catalogs). Triggers stay **workflow-level** unless reverses [D3](#d3--trigger-placement). Categories are FlowForge families (control flow, data, Kubernetes, SSH, scripts, HTTP/notifications). Cards show safe name, ports, required permissions, policy hints. Drag inserts defaults; **Add action** remains the guided path for target/credential/policy review.
...
- Compare stays redacted. Server compare is a gap, not a UI invention.

**R4 guardrails (— inherit on R4.1–R4.5):**

1. **One operate path.** Either the in-editor overlay **or** `/executions/{id}` detail as the dense operate surface — not both graphs duplicating chrome. R4.1 is list/filters that open existing detail; no inbox replay graph. R4.2 densifies the editor overlay only (remembered-open satellite + same-canvas highlight). R4.3 NDV I/O stays on the editor inspector.
2. **Loud `indeterminate`.** Never silent success when uncertain. R4.4 densifies cancel/retry/stop on inbox rows and the overlay; do not regress existing loud treatment on inbox or detail.
3. **Drafts never run.** Published `workflowVersionId` only.
4. **Compare is client-side** (`compareRedactedExecutions`). Ask only if a new projection is required. No `/replay` product route. R4.5 waiting → `POST /approvals/{id}/decide` on inbox rows and the overlay; requester cannot self-approve; no invented resume.

### Credentials

`/credentials`, `/credentials/new`, `/credentials/{id}` remain the vault. R5.1 densifies `/credentials` find-by-display-name (search/filter, useful columns, open existing detail). R5.2 densifies `/{id}` test / rotate / usage / deletion-impact at operate density on existing vault routes. R5.3 densifies selected-node NDV add via the masked wizard without abandoning the graph (return-to-editor): picker selects the new display name; YAML stores UUID only. Types stay `kubernetes`, `ssh_private_key`, `token`, `webhook_secret`, `provider`. Metadata only after submit.

**R5 security line (bake on R5.1; inherit on R5.2 and R5.3):**
...
Landed IA. Authoring path: **home → editor top bar → palette → inspector → YAML mode → runs drawer.**
...
Routes stay stable unless fold-in proves a rename is cheaper than teaching the old paths. **Prefer migrate in place** over a greenfield UI package.
...
| Editor | `/workflows/{id}` drawers | Same route; palette + NDV as satellites, not hidden-by-default | **Migrate** chrome; canvas engine upgrade only if cannot land primitives on the current graph |
...
| Membership / isolation | ADV-024 | Keep grant-gated; stop looking like the product | **Reshape** (R7.2): copy/placement off product chrome; Settings may link carefully; **do not** retire the grant |
...
| New studio origin or `/studio` | — | **Do not add** | Greenfield UI only if R2 spike fails (+) |

**Migrate vs greenfield ([D6](#d6--greenfield-ui-package) locked):** migrate `apps/web` App Router in place. Epic already forbade a second embed tree. A greenfield canvas package is in scope **only** as an R2 spike outcome if the current graph cannot do undo, multi-select, and decided layout without a rewrite. That spike is -owned and must not fork routes or contracts.
...
| --- | --- | --- | --- |
| Canvas-first editor | Viewport canvas, top bar, drawers | **Reshape** | Baseline to migrate *from*. Palette/NDV availability and graph primitives are the rewrite, not another chrome epic numbered UX.13+. |
...
| Membership / isolation | ADV-024 grant; exercise UX | **Reshape** (R7.2) | Keep fail-closed grant and negative tests; retire “this is the product” framing. Isolation success is a denial. |
| Local seed | `local` / `default` + demo vault in non-prod | **Keep** (R7.3) | Labeled **Example context** stays local-only. Trusted-dev header fallback is never rewrite login. Production boot-fail if forced on. Rewrite must not copy seed into k8s. |
| Ops surfaces | `/config`, OpenAPI, incident/retention docs | **Keep** | authority. Not a product home. |
| Action wizard | Guided add | **Reshape** | Keep policy/credential review; NDV becomes the everyday edit. |
| Auto-layout-only positions | Not persisted | **Replace** ([D1](#d1--canvas-layout-persistence) locked; API landed) | Persist optional non-authoritative `metadata.ui.layout`. Executor ignores it. Invalid/missing → auto-layout; never invent nodes/edges. |
...
| Earlier stacked operator page | Gone as product IA | **Retired** | Do not restore. The earlier inventory is historical. |
| `/replay` or second replay graph | Not present | **Do not add** | |
| Connector marketplace / `workflow.call` | Registry-disabled | **Keep disabled** | Separate epic if a later epic wants coverage. |
...
| --- | --- | --- | --- |
| D1 | Persist canvas positions | **Locked.** Optional non-authoritative `metadata.ui.layout` (executor ignores; invalid/missing → auto-layout; never invent nodes/edges). Schema/API landed; The UI still wires UI on. | A second source of truth; invalid layout must never invent nodes/edges |
...
- **D1 locked — persist layout:** optional, non-authoritative `metadata.ui.layout` is stored and returned but **is not** used for dispatch, policy, or port typing. Missing/invalid layout → auto-layout; never a guessed graph. Field spec: [§11.2](#112-control-plane-execution-and-credential-parity-gaps). Schema/API landed; The UI consumes it on.
- **D4 locked — do not replace:** stay on `flowforge/v1`. A new `apiVersion`, dual-read, export/import migration, and a versioned epic are out of scope unless records a reversal here.
...
**Additive gaps (do not invent UI routes):** see [§11.2](#112-control-plane-execution-and-credential-parity-gaps) for required-for-R# vs defer. Headline list:
...
| Local seed | Keep local-only; demo credentials stay placeholders. R7.3 keeps Example context labeled and does not promote trusted-dev as rewrite login. |
| Operator bookmarks | Keep `/workflows`, `/workflows/{id}`, `/embed/v1/…`. Redirects if a route is renamed. |
...
High-level phases only. Epics can be opened later from these rows. Do not create issues from this page. Each phase is one epic: one outcome and explicit in/out, with no hidden “also rewrite the API.”
...
| --- | --- | --- | --- | --- |
| **R1 — Charter freeze** | decisions D1–D6 recorded in this doc; stubs filled; this file is the rewrite source of truth | Docs + decision log only. No `apps/*`. R1 can close when this D1–D6 lock lands | Charter | Charter; this lock |
| **R2 — Editor interaction** | Opening a workflow feels like an n8n-class graph tool: palette and NDV available, graph primitives (undo/redo, multi-select, fit/snap), same `/workflows/{id}` + embed page | `apps/web` editor chrome/graph only. No draft execute. No YAML replacement. Layout persist via [D1](#d1--canvas-layout-persistence) `metadata.ui.layout` (schema/API landed). Greenfield canvas package only if a written spike fails ([D6](#d6--greenfield-ui-package)) | Editor chrome; layout field landed | R1 |
| **R3 — NDV and mapping** | Selected node is a FlowForge NDV: parameters, typed ports, credential pick/add, redacted last-run, validation/policy. Wizard remains the guided add | Inspector/wizard/adapters. Catalog fallback removal only when catalogs are complete. No vault plaintext. No new credential types | Inspector chrome; catalog gaps | R2 |
| **R4 — Executions density** | Workspace inbox + in-editor overlay match n8n-class operate-a-run: filter, overlay, NDV I/O, cancel/retry/stop, `indeterminate`, waiting → decide | Existing execution/approval routes. No `/replay`. Compare uses existing client diff until a compare route exists | Operate chrome; optional compare route | R3 (overlay/NDV), E5/E10 already on main |
| **R5 — Credentials in the workbench** | Vault is the credential store operators expect: find by display name, test/rotate/usage, NDV add without leaving the graph | `/credentials/*` + NDV wizard. No KEK in the browser. No `/config` merge unless a later decision asks | Vault chrome | R3 |
| **R6 — Activations and triggers** | Home + editor expose “this published version is active” for webhook/schedule (and manual start) without teaching three drawers | Trigger admin APIs already on main. **Activation confirmation** baked on R6.1 (inherit on the later slices): [D2](#d2--activation-model) compose enable + version pin, no new resource/aggregate; [D3](#d3--trigger-placement) workflow-level; drafts never run / never look live. change the read model only if a real gap appears — document it, do not invent one | Activation chrome; compose/read-model only | R1 D2, E10 on main |
| **R7 — Embed, tenancy, operator migration** | Rewrite chrome ships on `/embed/v1` with `session.embed`, ADV-024, CHIPS; operators leave exercise-shaped membership/isolation; local seed and ops docs still correct; E12.3 a11y contract extended | No ADV weakening. No Portal DB share. Membership/isolation stay grant-gated. Update docs and the harness only if a boundary moved (it should not) | Embed chrome | R2–R6 landed enough to mount |

**Out of this program (do not sneak into R2–R7):** marketplace connectors, `workflow.call`, OIDC login productization (unless a later decision adds D9), mobile app, screen-reader graph rewrite, replacing PostgreSQL or the worker model.

**Epic definition of ready:** copy the row, then add: operator outcome, in/out, parent (this doc §9), API/YAML/UI impact, authz/abuse, acceptance, tests, docs, rollback, telemetry. Use the [master plan](../master-implementation-plan.md) definition-of-ready checklist.
...
Locked 2026-09-11 in the FlowForge development group. R1 can close when this decision log lands.

### Product calls

| ID | Decision | Status |
| --- | --- | --- |
| [D1](#d1--canvas-layout-persistence) | Persist canvas layout as optional non-authoritative `metadata.ui.layout` (executor ignores; invalid/missing → auto-layout; never invent nodes/edges). Schema/API landed; stays open for UI. | **Locked** |
...
**Locked.** Persist canvas layout as optional non-authoritative `metadata.ui.layout` (executor ignores; invalid/missing → auto-layout; never invent nodes/edges). Schema/API landed; The canvas wires the canvas on.
...
** D5 hard line (R6.3):** one gesture may mint a published test version then start it — **never** run the unsaved/draft buffer. No draft execute path. No silent “test the open editor YAML.”
...
Answered in [§11.1](#111-ui-surfaces-and-operator-migration-notes). Summary:
...
- A11y: R7.4 extends UX.10 to NDV, palette, Runs overlay, activation chrome, and other R2–R6 satellites; still no SR graph rewrite.
- Touch/narrow: keep the inspector-first breakpoint (`touch-inspector-first`).

### Contracts

Answered in [§11.2](#112-control-plane-execution-and-credential-parity-gaps). Summary:

- D1 locked: field shape, validation, and “layout ignored by executor” tests for `metadata.ui.layout` (schema/API landed); executor/policy/ports ignore it. UI.
...
- Catalog fallback: when can delete `contract-fallback` for K8s/SSH/script/HTTP? → R3, when she ships against live catalogs only; those catalogs are already complete on `main`.
...
Paste drafts **under the matching heading**. Do not rename sections, do not move §1–§10, do not add a second IA. Do not link out to a tracker until those stories exist.

### 11.1 UI surfaces and operator migration notes



Fold-in of the standalone UI-surface draft (`docs/reference/rewrite-ui-surfaces.md`). **This subsection is the charter authority** for UI surfaces and operator migration. The standalone page is the expanded surface map — do not keep a second competing IA. Consume [§11.2](#112-control-plane-execution-and-credential-parity-gaps) contracts; do not invent routes.
...
§3–§6 already match landed chrome (home, canvas-first editor, YAML/runs drawers, inspector tabs, same-canvas overlay, vault by display name). R2.1 replaces hide-by-default library with a remembered-open satellite. R2.2 does the same for the NDV-style inspector shell. R2.3 adds labeled undo/redo for canvas graph edits. No table patch.

#### Locked (D1–D6)

Matches [§7](#7-security-and-tenancy-invariants) / [§8](#8-data-and-contract-strategy) and [§11.2](#112-control-plane-execution-and-credential-parity-gaps). **No conflict to record.** Recorded in [§10](#10-open-questions).

| ID | Locked (UI; no conflict) | Consequence |
| --- | --- | --- |
| **D1** | Optional non-authoritative `metadata.ui.layout` | Persist only via `metadata.ui.layout` (API landed). Missing/invalid → auto-layout. Never a second canvas file. Never invent nodes/edges. Executor ignores layout. wires the canvas. |
...
| Surface | On `main` after | Rewrite still needs (FlowForge chrome) |
| --- | --- | --- |
| **Home** | `/workflows` list/cards; client folder-prefix filters; first-class activation column (R6.2); create/import/duplicate/template all POST a **draft**; `?start=` / `?webhooks=` / `?schedules=` still work | Waiting / last-run density leftovers. First-class folder UX: [folder IA](flowforge-workflow-folders.md) (API + rail; tags out of that epic). Empty state: create / template / import — no developer fixtures. |
| **Editor / canvas** | `/workflows/{id}` viewport; invalid YAML never guesses a graph; undo/redo (R2.3); multi-select / fit / snap (R2.4); optional `metadata.ui.layout` on draft save/load (R2.5) | Minimap/alignment if they do not steal viewport. Touch stays a breakpoint, not a mobile app. |
| **Palette** | Left drawer, remembered-open satellite (R2.1); enabled catalog; **Add action** wizard; `/actions` is reference | Stronger category / recommended-from-upstream UX on the **same** enabled catalog. No marketplace. No disabled next/provider types. Triggers excluded (`rules.triggersAreWorkflowLevel`). |
| **NDV / inspector** | Right rail, remembered-open satellite (R2.2): type-specific parameters / `with` for cataloged core / Kubernetes / SSH / script / HTTP (R3.1); typed field-path mapping between ports (R3.2); pins / display-name credentials; workflow tabs Triggers / Versions / Pins; redacted last-run I/O at operate density when a run/step is in context (overlay or latest; R4.3 failures jump to the node); validation/policy density for the selected node (R3.3) | Catalog fallback removal (R3.4). No expression language. No `SecretField` in the rail. No branded NDV modal. |
| **Runs** | Remembered-open satellite (R4.2); overlay on **this** canvas; filter/list this workflow’s runs; skip-to-failed / skip-to-indeterminate; cancel / retry / emergency stop on inbox rows and the overlay (R4.4 Retry gated by `result.retry.allowed`; loud `indeterminate`); waiting → decide on inbox rows and the overlay (R4.5 `POST /approvals/{id}/decide`; no self-approval; no invented resume); **Open execution** → `/executions/{id}`. Workspace `/executions` inbox density (R4.1) on existing `status` / `workflowId` / `limit`. One operate path: inbox is not a second graph; overlay does not mount `ExecutionReplay`. NDV I/O (R4.3) stays on the editor inspector. | No `/replay`. Client compare only. |
| **Vault** | `/credentials/*` workbench find by display name (R5.1); detail test/rotate/usage/deletion-impact at operate density (R5.2); selected-node NDV add via masked wizard without leaving the graph (R5.3 return-to-editor; picker selects display name; YAML stores UUID) | Usage/deletion-impact without a secret surface in the inspector. Keep rotate/disable/test on vault routes. |
| **Activations** | Editor top bar + Triggers tab (R6.1) and `/workflows` home column (R6.2) compose “this published version is active” from enable + version pin. Home drawers still work. Drafts never look live. One-gesture **Test run** (R6.3) mints a published test version then starts it (D5 hard line: never the unsaved/draft buffer; no silent test of the open editor YAML). | No new activation resource. Drafts still never run. |
| **Embed / settings** | Same pages at `/embed/v1`; R7.1 mounts R2–R6 rewrite chrome after `GET /session` `session.embed` (Commands + Search on embed chrome; product children are the canonical pages). Settings holds session/health/OpenAPI + disclosed Developer samples. R7.2 reshapes membership/isolation off product chrome: Settings may link carefully; nav/search/Commands omit inaccessible capabilities; isolation success remains a denial; ADV-024 grant stays. R7.3 keeps labeled **Example context** + ops seed docs aligned with this IA; trusted-dev is never rewrite login. R7.4 extends UX.10 / E12.3 a11y to rewrite satellites | Migrate chrome with standalone. No second embed tree. Do not weaken the R7 hard line. |
...
Migration is from **today’s canvas-first chrome**, not from the pre-makeover stacked page.
...
#### Answers to [§10](#10-open-questions) — UI
...
3. **Foundation copy** — Settings keeps session/health/OpenAPI + disclosed Developer samples. R7.2 takes membership/isolation off product chrome: Settings may link carefully; they stay ADV-024 grant-gated; isolation success is a denial. R7.3 keeps Example context labeled and local-only; trusted-dev header fallback is never rewrite login.
4. **Embed** — R7.1 mounts R2–R6 rewrite chrome on the existing `/embed/v1` tree with `session.embed`. No Portal-specific cookie. No host-query authz. No second tree. **The R7 hard line** is baked in (`R7_HARD_LINE` in `apps/web/src/lib/rewrite-embed-mount.ts`) and **inherited by the later slices** — do not weaken: ADV-021 `session.embed` only (fail closed on `/embed/v1` without it); ADV-024 grant stays (reshape is R7.2); host `?tenant=` / `?workbench=` display-only; same `/embed/v1` mounts; issuer / frame-ancestor fail-closed stay; only if a boundary actually moves (it should not).
5. **A11y** — R7.4 extends UX.10 (Esc, focus return, no nested `<main>`, icon+text) to NDV, palette, Runs overlay, activation chrome, and other R2–R6 satellites. Still no screen-reader graph rewrite.
...
### 11.2 Control-plane, execution, and credential parity gaps



Fold-in of the standalone control-plane draft (`docs/architecture/rewrite-n8n-parity-control-plane.md`). **This subsection is the charter authority** for control-plane / execution / credential gaps. The standalone page can be superseded (redirect or retire) now that R1 is freezing this file; do not keep a second competing contract. UI-surface notes ([§11.1](#111-ui-surfaces-and-operator-migration-notes) and `docs/reference/rewrite-ui-surfaces.md`) consume these contracts — they do not invent routes.
...
UI already consumes most of this (UX.1–UX.12). Known **API** density holes already filed as non-blocking: no execution-vs-execution compare route; no replay-projection endpoint (client joins version YAML + steps).

#### Contract locked (D1–D6)

Matches [§7](#7-security-and-tenancy-invariants) / [§8](#8-data-and-contract-strategy). No conflict to record. Recorded in [§10](#10-open-questions); this is the control-plane contract those locks adopt.

| ID | Locked (no conflict with) | Contract |
| --- | --- | --- |
| **D1** | Optional non-authoritative `metadata.ui.layout` | Additive optional object on `flowforge/v1`. API stores and returns it. **Executor, `POST /policy/evaluate`, port typing, and dispatch ignore it.** Missing/invalid → auto-layout; never a guessed graph; never invent nodes/edges. See field spec below. **Landed (schema/API).** The canvas wires canvas persist on. |
...
**D2 compose (no new resource):** a workflow is “active” when a **published** version has at least one enabled webhook or schedule pin. Manual start is on-demand, not activation. Home/editor chrome may *label* that compose; it must not imply the draft is live. Confirmed for R6 — bake on R6.1; inherit on the later slices.
...
| --- | --- | --- | --- |
| Canvas layout persist | Extension: `metadata.ui.layout` | **R2** ([D1](#d1--canvas-layout-persistence) locked) | Field spec above. Schema/API landed. UI still open. No second persisted format. |
| Graph primitives / NDV chrome | None (API) | R2 / R3 |. Catalogs already typed. |
| Catalog `contract-fallback` | None (API). UI may drop fallback in R3 | **R3** | Core/K8s/SSH/script/HTTP metadata is complete on `main`. deletes fallback when she ships live-catalog-only. No new credential types. |
...
| Folders / projects as resources | **Promoted** — nested folder resource, not tags-first | **Post-R7** ([folder IA](flowforge-workflow-folders.md)) | Home folder filters are a **client-side** name/slug prefix today. First-class: server-backed `folderId` on `workflows` (not a second tenancy axis). Do not bypass workspace RLS. Tags remain out of that epic. |
...
| Embed / tenancy / CHIPS | **Keep** | R7 | `session.embed`, ADV-024, host-issuer bind, partitioned cookies. standby: docs/harness only if a boundary moved (it should not). |
...
| Standalone OIDC | Greenfield-ish extension | **Out of this program** unless a later decision adds D9 | Must issue the existing `ff_session` + CSRF pair. Header identity stays non-prod fail-closed. |
...
#### Answers to [§10](#10-open-questions) — contracts

1. **D1 field** — `metadata.ui.layout` as specified above; executor-ignore tests landed with the schema PR. read `summary.ui.layout` / YAML `metadata.ui.layout` on. from the API PR.
...
4. **Catalog fallback** — may delete `contract-fallback` for K8s/SSH/script/HTTP in R3; API metadata is already on `main`.
...
- [Frontend UI](../reference/frontend-ui.md) — current baseline
...
- Standalone draft (source for §11.2; **can be superseded** after R1): `docs/architecture/rewrite-n8n-parity-control-plane.md` on — not copied here, so this charter does not grow a second authority
- UI-surface draft (expanded tables for [§11.1](#111-ui-surfaces-and-operator-migration-notes)): [rewrite-ui-surfaces.md](../reference/rewrite-ui-surfaces.md) (also)
