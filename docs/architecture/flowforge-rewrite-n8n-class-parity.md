...
| **Editor / canvas** | `/workflows/{id}` viewport; auto-layout only; invalid YAML never guesses a graph; undo/redo (R2.3 / #236); multi-select / fit / snap (R2.4 / #237 — keep #237 open) | Minimap/alignment if they do not steal viewport. Layout persist via [D1](#d1--canvas-layout-persistence) `metadata.ui.layout` (API landed; Chloe #238). Touch stays a breakpoint, not a mobile app. |
