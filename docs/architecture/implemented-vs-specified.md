...
| **Workflow soft-delete** | Delete hides a workflow, unpublishes it, disables triggers and schedules, and reserves the slug. No hard purge and no restore. | **Runs today (API).** `DELETE /api/v1/workflows/{workflowId}` returns `204`. Editors, workspace admins, and the workflow owner may delete. Viewers who are not the owner get `403`. No view access gets `404`. Queued or running executions get `409` `workflow_has_active_executions`. A reserved slug gets `409` `workflow_slug_reserved`. Responses include `capabilities.delete`. No delete route on `/embed/v1`. The browser confirm UI is separate. | **Runs today** (API) |
