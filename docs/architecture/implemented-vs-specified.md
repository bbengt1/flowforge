...
| **HMAC job / script keys** | Every boundary fails closed; production secrets required. | `JOB_BINDING_SECRET` and `SCRIPT_SIGNING_KEY` **default to per-process random** (including malformed values). Horizontal scale and rolling restart break tickets and published script signatures. `EMBED_SIGNING_KEY` already boot-fails in production. | **Partial** — fail-open (gap **B1**; G.0.3) |
...
| **OpenAPI / tracing (E1.2)** | Generated OpenAPI; API-to-worker flows traced by correlation ID. | OpenAPI is **hand-written**. `X-Request-ID` exists; **no trace store, sampling, or worker span propagation.** | **Specified only** (gaps **B5**, **C1**) |
