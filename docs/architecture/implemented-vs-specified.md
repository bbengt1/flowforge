...
| **OIDC / SSO / MFA / SCIM** | Enterprise identity; architecture mentions host/OIDC subjects. | OIDC Authorization Code + PKCE (`POST /oidc/start`, `POST /oidc/callback`) and TOTP MFA on `platform.administer` and `credential.*` are API contracts. No SAML, SCIM, or durable account lockout. Chrome follows separately. | **Partial** (gap **A2** API; SCIM still deferred) |
...
| OIDC + MFA | Enterprise IdP | `POST /api/v1/oidc/start`, `POST /api/v1/oidc/callback`, `GET/POST /api/v1/session/mfa` | Authorization Code + PKCE mints the same `ff_session`. TOTP gates privileged grants on local-login and OIDC only. No IdP-admin API. SCIM still deferred. |
...
- A claim that later identity work has shipped. The production runner (G.1.1), leader-elected scheduler (G.1.3), encrypted backup CronJob (G.1.4), durable artifact storage (G.1.5), machine/service-principal credentials (G.1.6), and web Deployment (G.1.7) have. OIDC Authorization Code + PKCE and TOTP MFA (G.2.1 API) have. SCIM (G.2) has not. Login and MFA chrome follow separately.
