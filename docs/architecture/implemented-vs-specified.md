...
| **Retention purge / backups** | 90-day execution / 365-day audit purge; encrypted `pg_dump`. | The leader calls retention purge on the same interval (legal holds stay). `scripts/backup/encrypt-pg-dump.sh` exists. **No backup CronJob** — operators still schedule the encrypt script (G.1.4). | **Partial** (purge runs; backups gap **A3**) |
...
- A claim that the rest of G.1 has shipped. The production runner (G.1.1) and the leader-elected scheduler (G.1.3: dispatch, recover, purge) have. Encrypted backup CronJob (G.1.4), durable artifact storage (G.1.5), and the web Deployment (G.1.7) have not.
