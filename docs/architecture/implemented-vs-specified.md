...
| **Artifact durability** | Queue, leases, execution state, audit, and artifact metadata survive pod loss. | Metadata is in PostgreSQL. Payloads are envelope-encrypted and written to a configurable S3-compatible store (compose MinIO on `minio_data`). Production-locked processes refuse filesystem and memory stores. | **Runs today** (G.1.5 / gap **B3**) |
