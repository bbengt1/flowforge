...
Executable on compose today: **6 of 18** defined node types (`flow.condition`, `flow.stop`, `flow.fail`, `data.set`, `data.map`, `data.validate`). Non-executable on that worker: provider nodes, `flow.delay`, plus registry-disabled `data.filter` / `data.merge` / `data.sort` / `flow.join` / `flow.parallel` / `flow.switch`. `flow.approval` waits in the API. The production runner also evaluates those six core nodes, parks `flow.approval`, and parks `flow.delay` with `WaitJob` until the duration elapses (not an in-process sleep). Compose still fails `flow.delay`. A step retry is refused unless the run is `failed` or `indeterminate` and the latest attempt of that step started and ended `failed` or `indeterminate`; cancel and `workflow_deleted` close pending approvals in the same transaction.
