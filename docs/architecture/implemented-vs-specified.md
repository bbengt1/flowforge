...
- Do **not** say clusters, SSH hosts, webhooks, or script Jobs are reachable from `deploy/k8s` until an operator adds provider egress (including the control-plane API CIDR for script Jobs). Default-deny stays. The script-runner image and Job template are shipped; the dial is not opened by default.
