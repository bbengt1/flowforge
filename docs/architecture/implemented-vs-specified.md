...
| **Lease recovery** | Expired claims become `indeterminate` via fencing; `POST /api/v1/jobs/recover`. | The same leader calls recover on the interval. The endpoint remains. A stale HMAC `jobToken` still cannot complete. | **Runs today** (G.1.3 / #434) |
| **Retention purge / backups** | 90-day execution / 365-day audit purge; encrypted `pg_dump`. | The leader calls retention purge on the same interval (legal holds stay). `deploy/k8s/backup-cronjob.yaml` runs `scripts/backup/run-encrypted-backup.sh` daily (logical RPO 24h) and writes an FFB1 integrity manifest. `flowforge-wal-archive` seals WAL (PITR RPO 300s) and `flowforge-pitr-base` seals a physical base backup. Ciphertext lands in `BACKUP_S3_BUCKET` under `flowforge-db/`. Rehearsals are `scripts/backup/rpo-rto-rehearsal.sh` and `scripts/backup/manifest-pitr-rehearsal.sh` in E12.2 CI. | **Runs today** (purge G.1.3; backups G.1.4 / #435; manifest + PITR G.2.7 / #452) |
...
| **HA / web on Kubernetes** | Isolated worker pods + UI in cluster. | `deploy/k8s` runs API, web (`flowforge-web`, `GET /` probes), and runner at `replicas: 2` with PDB (`maxUnavailable: 1`), HPA (minimum 2), preferred hostname anti-affinity, `preStop`, and termination grace. SIGTERM resigns the scheduler lease before HTTP drain. `JOB_BINDING_SECRET` and `SCRIPT_SIGNING_KEY` are the shared Secret. Compose `/usr/local/bin/worker` is not deployed. | **Runs today** (G.2.6 / #451; web G.1.7 / #438). A managed-Postgres manifest is still not in `deploy/k8s`. |
