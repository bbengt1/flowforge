...
| **Retention purge / backups** | 90-day execution / 365-day audit purge; encrypted `pg_dump`. | The leader calls retention purge on the same interval (legal holds stay). `deploy/k8s/backup-cronjob.yaml` runs `scripts/backup/run-encrypted-backup.sh` daily (RPO 24h); ciphertext lands in `BACKUP_S3_BUCKET` under `flowforge-db/`. RPO/RTO rehearsal is `scripts/backup/rpo-rto-rehearsal.sh` in E12.2 CI. | **Runs today** (purge G.1.3; backups G.1.4 / #435) |
...
- A claim that later identity work has shipped. The production runner (G.1.1), leader-elected scheduler (G.1.3), encrypted backup CronJob (G.1.4), durable artifact storage (G.1.5), machine/service-principal credentials (G.1.6), and web Deployment (G.1.7) have. OIDC Authorization Code + PKCE and TOTP MFA (G.2.1 API) have. SCIM 2.0 provision/deprovision and durable account lockout (G.2.2 API) have. SAML has not. Login and MFA chrome follow separately.
