...
- [First-run bootstrap gate](architecture/flowforge-first-run-bootstrap.md): B.1 `GET /api/v1/bootstrap` + B.2–B.5 / B.7 wizard APIs (`persistence`, `admins`, `public-url`, `tls` create/upload/skip + `MarkComplete`). B.6 chrome landed; Chloe Skip chrome is follow-up. Standalone wizard only — never `/embed/v1`.
