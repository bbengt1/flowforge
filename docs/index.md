...
- [First-run bootstrap gate](architecture/flowforge-first-run-bootstrap.md): B.1 `GET /api/v1/bootstrap` + B.2 `POST /api/v1/bootstrap/persistence` + B.3 `POST /api/v1/bootstrap/admins` + B.4 `POST /api/v1/bootstrap/public-url` + contract map for B.5–B.6. Standalone wizard only — never `/embed/v1`.
