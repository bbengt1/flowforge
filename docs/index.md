...
- [First-run bootstrap gate](architecture/flowforge-first-run-bootstrap.md): B.1 `GET /api/v1/bootstrap` + B.2–B.5 wizard APIs (`persistence`, `admins`, `public-url`, `tls` + `MarkComplete`). B.6 chrome is next. Standalone wizard only — never `/embed/v1`.
