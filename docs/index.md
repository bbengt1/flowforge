...
- [Workflows home folder hierarchy](architecture/flowforge-workflow-folders.md): Brent-promoted IA for nested, server-backed folders on `/workflows` (per workspace). Docs-only; Arie opens the epic. Not tags-first; not client-only persistence.
