...
**Feeds:** Gracie’s master rewrite vision doc (not yet the authority
for this slice). Treat the tables below as Chloe’s draft for that
vision — Gracie owns sequencing, epic cut, and the combined rewrite
plan.
...
| **Canvas** | `/workflows/{id}` takes the viewport. Pan, zoom, select, compatible port connect, library drag-drop (defaults). Auto-layout only — **not** a persisted UI format. Invalid YAML never draws a guessed graph. Node state is icon + text. Keyboard: focus canvas, zoom/reset; selecting a node announces enough to use the inspector. | Fit-to-workflow, snap-to-grid, minimap, multi-select, alignment, undo/redo (documented as aspirational, not landed). Optional **hint** positions that still serialize through YAML (never a second format). Richer empty-canvas **+** without opening a third app. Touch remains a `max-width: 767px` inspector-first **breakpoint**, not a mobile app (`touch-inspector-first` gap). |
| **Node library** | Left drawer, **hidden on first paint**. **Library** or canvas **+** opens the enabled catalog. **Add action** opens the wizard (type → authorized target/credential → configure → map ports → review). `/actions` is the catalog **reference**, not a third app. Triggers excluded (`rules.triggersAreWorkflowLevel`). Catalog 403 / empty list fail closed. No invented `INTEGRATION_ACTIONS_ENABLED` toggle. | Faster “add from empty canvas” (palette stays one click, not a hunt). Stronger category / recommended-from-upstream-port UX on the **same** enabled catalog. Keep `/actions` as reference. Do not add a connector marketplace or disabled next/provider types. |
| **Inspector (NDV-like)** | Right rail. **Node:** name, `with`, pins, credentials by **display name** (pick or add; secret entry stays in the masked wizard). **Workflow:** tabs Triggers / Versions / Pins (UX.9). **Edge:** port compatibility. **Validation:** live region; errors link to a node or YAML path. **Last run** (UX.11): redacted input / output / logs for the selected node when a run is selected. Rail never shows `SecretField` / plaintext / rotate. | A focused node inspector that still feels like “open this step”: parameters, pins, credential, and last-run I/O in one place without a stacked operator page. Typed mapping that stays field-path only (**no expression language**). Read-only when `workflow.edit` is missing. Do **not** ship a branded NDV modal, expression editor, or secret surface in the rail. |
...
2. The library is a **drawer**, hidden on first paint — not a
   permanent 18rem column.
...
| --- | --- | --- |
| Auto-layout-only canvas | Optional layout tools (fit, snap, multi-select, undo/redo) that still save YAML | A persisted canvas file or n8n-like binary graph. |
| Hidden-on-first-paint library | Faster add-from-canvas on the **same** enabled catalog | A marketplace or always-on cloned sidebar. |
| Inspector rail + last-run panel | Deeper focused node inspector (parameters + pins + credential + redacted I/O) | A branded NDV, expression editor, or SecretField rail. |
...
## How this feeds Gracie’s master rewrite doc

Chloe’s slice is the **UI surface + migration** input. Gracie owns
the combined rewrite vision (epics, sequencing, API/worker cuts).

Suggested paste targets for that vision:

1. The surface map table (today vs parity).
...
Upcoming rewrite epics should link here rather than re-inventory
`main`. Do not treat this page as permission to open production UI
PRs or to close #195-era stories.
...
| [master-implementation-plan.md](../master-implementation-plan.md) | MVP epic backlog (E1–E12). Rewrite vision is separate and Gracie-owned. |
