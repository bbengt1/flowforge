...
| Membership / Isolation | `/membership`, `/isolation` | R7.2 (#277 — **keep #277 open**): off product chrome; Settings may link carefully. ADV-024 grant only. Isolation success is a **denial**. |
...
| Save draft / Publish / Start published / Test run | Same verbs. Publish is last **saved** draft only. Start lists **published** versions only. **Test run** mints a published test version then starts it (D5). |
...
| Vault list → new → detail | `/credentials` routes remain. R5.1 densifies list find-by-display-name. R5.2 densifies detail test/rotate/usage/deletion-impact. R5.3 densifies NDV add without leaving the graph (return-to-editor). |
