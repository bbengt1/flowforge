...
**Owner:** (UI surfaces, operator migration, keep/replace/retire).
**Folded into:** charter §11.1
(PR). That subsection is the rewrite authority for this slice;
...
### What already changed in (do not regress)
...
(including Commands + Search) after `session.embed`. + R7
hard line is baked on and inherited on — do not weaken
...
| Prefix-encoded folders on home | First-class nested folders when the API exists (folder IA; tags out of that epic) | Client-only fake trees that hide the slug. |
...
| --- | --- |
| Pre- stacked operator page (YAML + palette + inspector + fixtures always on) | Canvas-first satellites. Already landed; do not bring the stack back. |
...
permission to open production UI PRs or to close stories.
...
| flowforge-rewrite-n8n-class-parity.md | Rewrite charter. **§11.1** is the fold-in authority for this page. |
| flowforge-ux-laws.md | Post-R1–R7 chrome polish (Laws of UX). Docs-only; does not reopen this IA. |
| master-implementation-plan.md | MVP epic backlog (E1–E12). Rewrite vision is the charter; open epics later. |
