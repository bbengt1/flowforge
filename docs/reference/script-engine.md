...
Inputs arrive as validated JSON against the declared `inputSchema` (object root) and a 16 KiB cap; secret keys and plaintext credentials are rejected before inject. Outputs must meet the declared `outputSchema` (object root) and the same size limit, then are redacted before persist/audit. Non-object root schemas (`array`, `string`) are `invalid-schema` at publish and execute — validated output is never rewritten as `{"value": parsed}`. Absent prior output on a later attempt stays `indeterminate` — never a blind re-run. Environment variables expose only an allowlisted runtime context (`FLOWFORGE_*`). Credential handles are short-lived, scoped, and never serialized as plaintext.
