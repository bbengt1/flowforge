...
Runtime profiles stay ops-config `kind=runtime_profile` (E4.2): digest-pinned `imageDigest` + `dependencyLockDigest`, required `limits.{cpuMillis,memoryMib,timeoutSeconds,processes}`, optional `egress.destinations` (default-deny; DNS is constrained). Python runs under `python3` in the script-runner image. Go is compiled inside that image (`GOPROXY=off`, stdlib only) after the in-process signature check; CI's `StubBuilder` still does not invoke `go build`. Full containers are not started in `go test` — `HarnessRuntime` asserts UID / read-only root / dropped caps / `no_new_privs` / metadata / egress / limits / package-install, and `KubernetesJobRuntime` tests record the Job manifest. Production pods use `deploy/kubernetes/script-runner-*.yaml` (non-root 65532, no SA token, no docker.sock, default-deny NetworkPolicy + kube-system DNS). The runner NetworkPolicy does not open the control-plane API until an operator adds that CIDR.
