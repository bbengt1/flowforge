# E12.1 UI/embed evidence

This is the Hit-list proof for
[e12-security-verification.md](../e12-security-verification.md) the UI map.
The harness/CI stays the API/engine authority. These rows only
...
| --- | --- | --- |
| Embed exchange + replay | `/embed/v1` · `EmbedExchangeGate` | the embed UI contract tests (iframe src, postMessage/form body-only, same-assertion replay `409`, GET `/session` chrome) plus existing `embed-client.test.ts` / ADV-013 checklist |
| Session chrome | `EmbedChrome`, `SessionStatusChip`, `SessionExpiryBanner` | Same contract (401 → existing stale chrome; expired chip/banner) plus `session.test.ts` / `session-client.test.ts` / `session-embed-contract.test.ts` |
| Expired approval | `/approvals` · `ApprovalValidityBanner`, `ApprovalDecideControls` | contract + `approval.test.ts` (`approvalValidityBannerState` / `approvalDecideControlsState`) |

Machine pointer: [embed-ui-last-run.json](embed-ui-last-run.json).
...
# or only the checklist:
node --experimental-strip-types --test the embed UI contract tests
# from apps/web
```
