...
| `execution_edges` | `execution_id`, `from_node`, `from_port`, `to_node`, `to_port`, `required`, `resolved`, `satisfied` | One row per published edge, pinned with the run (`000034_execution_edges.sql`). Unique per path. `required` is true for an edge out of `flow.approval`. An edge resolves once. FORCE RLS. |
