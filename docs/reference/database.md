...
| `instance_bootstrap` | singleton `id='default'`, `complete`, `skipped`, per-step ready flags, server-only `public_base_url`, status-only `tls_mode` | B.1 (`000024_instance_bootstrap.sql`). First-run wizard gate. Not workspace-owned; **no FORCE RLS** (identity-substrate class). Never stores secrets, KEK, passwords, or private keys. `GET /api/v1/bootstrap` returns flags only — never `public_base_url`. Localseed / migrate-backfill mark complete when admin+workspace already exist. |
