...
| `instance_bootstrap` | singleton `id='default'`, `complete`, `skipped`, per-step ready flags, server-only `public_base_url`, status-only `tls_mode` | B.1 (`000024_instance_bootstrap.sql`). First-run wizard gate. Not workspace-owned; **no FORCE RLS** (identity-substrate class). Never stores secrets, KEK, passwords, or private keys. `GET /api/v1/bootstrap` returns flags only — never `public_base_url`. B.5 writes cert/key to `TLS_CERT_FILE` / `TLS_KEY_FILE`; B.7 skip writes no files. This table keeps `tls_ready` + `tls_mode` only (`self_signed` \| `uploaded` \| `skipped` \| `local_http` \| `none`). Localseed / migrate-backfill mark complete when admin+workspace already exist; wizard `MarkComplete` is B.5 / B.7 only. |
...
| `execution_steps` | `id`, `execution_id`, `node_id`, `node_type`, `attempt`, `status`, `unresolved_incoming`, `lease_id`, `fencing_token`, `idempotency_key`, `policy_snapshot`, `target_snapshot`, `input_redacted`, `output_redacted`, `error_redacted`, timestamps | Unique `(execution_id, node_id, attempt)`; never store secret/plain raw output. `unresolved_incoming` is how many incoming edges have not resolved. `pending` and `skipped` are step statuses. `skipped` is finished and does not fail the run. |
| `execution_jobs` | `id`, `execution_step_id`, `status`, `available_at`, `lease_expires_at`, `heartbeat_at`, `worker_id`, `fencing_token`, `attempt`, `traceparent`, `tracestate` | Durable dispatch record; at most one active claim for a step attempt. `blocked` is not claimable. `traceparent` / `tracestate` (G.2.4, `000030_job_trace_context.sql`) are validated W3C correlation fields, nullable, and omitted from browser JSON. |
| `execution_edges` | `execution_id`, `from_node`, `from_port`, `to_node`, `to_port`, `required`, `resolved`, `satisfied` | One row per published edge, pinned with the run (`000034_execution_edges.sql`). Unique per path. `required` is true for the default AND join and false when the target node sets `join: any`. An edge resolves once. FORCE RLS. |
...
Workers claim one eligible job and lock that row with `FOR UPDATE` (`POST /api/v1/jobs/claim`). A claim requires job status `queued` (not `blocked`), `unresolved_incoming = 0` on the step, and an active execution. It is a column check, not a graph walk. A claim increments and returns `fencing_token` plus an HMAC job ticket bound to workspace, workflow version/digest, policy digest, and expiry. Every heartbeat, completion, and result write requires the matching active lease and token. A stale worker cannot overwrite a later worker's result. Lease expiry (E5.2, no provider verification hook yet) marks the job/step/execution `indeterminate`, not a silent retry. Cancellation is idempotent and requires `execution.cancel`. Retry creates a new `(node_id, attempt)` row only for retry-safe core nodes.

Execution input/output data is schema-limited and redacted before persistence. Large outputs, logs, and generated files are redacted/scanned before upload to encrypted object storage; an artifact that cannot be safely redacted is rejected rather than retained. `storage_ref` is an internal opaque locator, never a client-supplied URL or bucket/key. The object key is `{tenant}/{workspace}/{ref}` (UUIDs only); `ARTIFACT_S3_PREFIX` is rejected, and object metadata does not carry filenames or credentials. A draft execution (no published version id) cannot attach a run artifact. Artifact download authorization rechecks workspace and retention on **every** grant and stream request, then returns a short-lived same-origin grant (`/api/v1/artifact-downloads/{id}`); object-store credentials and durable public URLs are never returned. Retention purge deletes metadata and the encrypted object payload; a legal hold preserves both and writes `artifact.legal_hold.*` / `artifact.retention.held` audit rows. Objects live in an S3-compatible bucket (`ARTIFACT_S3_*`, compose MinIO), encrypted with the vault KEK before upload. Production-locked processes refuse `ARTIFACT_STORE_DIR` and in-process memory; those remain non-production fallbacks. Credential material, raw kubeconfig, SSH keys, access tokens, command lines containing secrets, and unredacted provider responses cannot be written to execution, artifact, or audit storage.
...
- Use pooled connections, query timeouts, migration serialization, and measured connection/query/lock/vacuum headroom before increasing worker replicas. Application-pool checkout (`PrepareConn`) now sets `statement_timeout` / `lock_timeout` (G.0.11 C2). The initial capacity target is at least 2x observed peak for connection pool, write throughput, queue lag, and storage growth. E12.2 proves that target with a fail-closed harness (`scripts/e12-resilience-suite.sh`) and last-run JSON under [e12-resilience-evidence](e12-resilience-evidence/README.md). See [e12-resilience-capacity.md](e12-resilience-capacity.md). The production API pool is `DefaultMaxConns=8` per pod. `deploy/k8s` HPAs allow 6 API replicas and 4 runner replicas; size Postgres `max_connections` for `(api max + runner max) × 8` plus backup and admin headroom before raising those ceilings. Concurrent migrate runners serialize on advisory lock `881726401`. The in-process scheduler holds a separate session advisory lock `881726402` on one application-pool connection (`SET ROLE flowforge_app`); only that replica ticks. If the session no longer holds the lock, it stops immediately. The lock does not bypass FORCE RLS.
