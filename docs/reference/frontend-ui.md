...
| First-run wizard | standalone shell gate · Settings `#bootstrap` | B.6 / #339 — **keep #339 open**. B.7 / #347 — **keep #347 open**. `GET /api/control-plane/bootstrap` from standalone only. `incomplete` → wizard (persistence → first admin → public URL → TLS, fail-closed). TLS offers Create / Upload / **Skip for now** (loud HTTP-until-Settings; `{action:"skip"}` only). `complete` or `401` → `/workflows`. Never on `/embed/v1`. After complete, Settings surfaces status (including `tls.mode=skipped`) and links out (users → `/membership`); the wizard does not remount. No browser-held secrets. |
