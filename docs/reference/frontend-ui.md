...
- **Explorer select/open (X.3 / #382 — keep #382 open; keep #379 open):** dense content-pane rows (name, published/activation, last-run as scan ends — no stats/Personal/link-count). Single-click selects a folder-child or workflow row without navigating. Double-click / Enter opens: workflow → editor; folder → expand/select in the tree and content pane. Selected-row highlight uses V.1 teal accent on charcoal. Right-click selects the target so X.2 menus stay. Arrow keys move the highlight when the list is focused. Multi-select deferred. Same `WorkflowHome` on embed (full parity is X.5).
- **Explorer empty / Unfiled teaching (X.4 / #383 — keep #383 open; keep #379 open):** empty folder and empty Unfiled differ on Explorer chrome. A real folder teaches create here / move (delete only when empty — refuse-if-nonempty). Unfiled stays virtual and never looks deletable or renamable. Drafts-do-not-run chrome stays loud on empty/home create surfaces. X.2 empty-pane context menu (New folder / Create / Import) stays. F.5 / O.3 teaching verbs carry forward; Overview card chrome (O.3 / #327 — **keep #327 open**) remains the teaching contract. Same `WorkflowHome` (full embed parity is X.5).
