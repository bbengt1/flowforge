...
- **Login/bootstrap:** trusted-dev `POST /api/v1/session` with `{issuer, external_subject, display_name?}`. `201` `{session,principal,csrf_token}` plus `ff_session` / `ff_csrf`. Local/dev only — never rewrite login. Production identity is `POST /embed/exchange`. First-run wizard gate is `GET /api/v1/bootstrap` (B.1 / #334) plus step 1 `POST /api/v1/bootstrap/persistence` (`{confirm:true}`; B.2 / #335), step 2 `POST /api/v1/bootstrap/admins` (`{issuer, external_subject, display_name?}`; B.3 / #336), step 3 `POST /api/v1/bootstrap/public-url` (`{publicBaseUrl}`; B.4 / #337), and step 4 `POST /api/v1/bootstrap/tls` (`{action:"create-self-signed"}` or `{action:"upload", certPem, keyPem}`; B.5 / #338) — standalone only, never `/embed/v1`. B.5 marks `complete`; do not remount the wizard. Do not POST a password until local login lands; never keep passwords, PEMs, or keys in `localStorage`. Status responses never echo the public URL, PEM, or private key (`steps.tls.mode` is status-only). See [first-run bootstrap](../architecture/flowforge-first-run-bootstrap.md). Chloe consumes it in B.6.
