...
| Canvas editor | `/workflows/{id}` · same page `/embed/v1/workflows/{id}` | Viewport canvas + sticky top bar. Undo/redo for graph edits (R2.3 / #236 — **keep #236 open**). Top bar states **Editing a draft** and which **published** version is active (or `not active`) (UXL.2 / #289 — **keep #289 open**; R6.1 / #270 — **keep #270 open**). Save draft, Publish, Test run, and Start published show **pending immediately**, then success or error (UXL.3 / #290 — **keep #290 open**). Canvas stays interactive while those requests run. Drafts never look live. Not a second studio app. |
