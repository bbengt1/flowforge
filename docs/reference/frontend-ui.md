...
| `POST` | `/api/v1/approvals/{approvalId}/decide` | `approval.decide` | yes | `{decision:"approved"\|"rejected", note?}`. No self-approval. Stale binding is `409`. A closed approval (`status=canceled`, `closeReason` `run_canceled` or `workflow_deleted`), or a gate that is no longer `waiting`, is `409` `approval_closed`. Nothing is recorded as approved. |
