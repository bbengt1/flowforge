...
- Row actions: Open editor, **Test run** (copy names “publish a test version, then start it”; when the role can publish and execute — mints a published test version from the last saved draft, then starts it), Start published (when a published version exists; otherwise explains it needs a published version), Webhooks / Schedules when the role can see those triggers, Last run. Activation status uses the same wording as the editor (`published vN is active` / `not active` / `Draft — not live`) and opens `/workflows/{id}#activation`. Home query drawers (`?start=`, `?webhooks=`, `?schedules=`) still work; they are not the test-run path.
...
| Surface | Behavior |
| --- | --- |
| List column | **Activation** after the workflow name: `published vN is active` / `not active` / `Draft — not live` (same words as the editor; UXL.2 / #289 — **keep #289 open**). Opens `/workflows/{id}#activation`. Never paints a draft as live. |
