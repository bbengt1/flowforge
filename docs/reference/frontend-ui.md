...
| Credential vault | `/credentials`, `/new`, `/{id}` | Workbench find (R5.1 / #264 — **keep #264 open**): display-name search + type/status/tag filters, useful columns, open-to-detail. Detail operate (R5.2 / #265 — **keep #265 open**): test / rotate / usage / deletion-impact on existing vault routes; disable/enable stay clear; rotate returns display-name + UUID only. Test and rotate show **pending immediately**, then success or error (UXL.3 / #290 — **keep #290 open**). Selected-node NDV add (R5.3 / #266 — **keep #266 open**): guided masked wizard without abandoning the graph (return-to-editor); picker selects the new display name; YAML stores UUID only. Gracie R5 security line: no KEK in the browser; display-name + UUID only; secrets never in YAML / search / analytics; unexpected plaintext is a contract bug (strip + stop). Dedicated routes remain. Wizard stays add; NDV stays edit/pick. |
