...
- **ssh.run wizard:** `retryPolicy.maxAttempts` defaults to 0. `maxAttempts>0` requires profile `retrySafe` **and** a declared verification probe. Otherwise the wizard says the step can't be retried (`invalid-verification` when the probe is missing). No affordance that implies a blind repeat.
