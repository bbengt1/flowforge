...
| Credential vault | `/credentials`, `/new`, `/{id}` | Workbench find (R5.1 / #264 — **keep #264 open**): display-name search + type/status/tag filters, useful columns, open-to-detail. Gracie R5 security line (inherit #265 / #266): no KEK in the browser; display-name + UUID only; secrets never in YAML / search / analytics; unexpected plaintext is a contract bug (strip + stop). Dedicated routes remain. Inspector create reuses the masked wizard. |
...
Operator routes (Chloe, E4.1 + R5.1): `/credentials` (display-name find), `/credentials/new` (wizard), `/credentials/{id}` (detail). Contract adapter: `apps/web/src/lib/credential-contract.ts`. Unexpected secret fields on API responses are stripped and treated as a contract bug.
