...
- **Login/bootstrap:** trusted-dev `POST /api/v1/session` with `{issuer, external_subject, display_name?}`. `201` `{session,principal,csrf_token}` plus `ff_session` / `ff_csrf`. Local/dev only — never rewrite login. Production identity is `POST /embed/exchange`. First-run wizard gate is `GET /api/v1/bootstrap` (B.1 / #334) plus step 1 `POST /api/v1/bootstrap/persistence` (`{confirm:true}`; B.2 / #335) — standalone only, never `/embed/v1`. See [first-run bootstrap](../architecture/flowforge-first-run-bootstrap.md). Chloe consumes it in B.6.
