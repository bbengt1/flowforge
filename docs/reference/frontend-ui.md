...
| First-run wizard | standalone shell gate · Settings `#bootstrap` | B.6 / #339 — **keep #339 open**. `GET /api/control-plane/bootstrap` from standalone only. `incomplete` → wizard (persistence → first admin → public URL → TLS, fail-closed). `complete` or `401` → `/workflows`. Never on `/embed/v1`. After complete, Settings surfaces status and links out (users → `/membership`); the wizard does not remount. No browser-held secrets. |
...
- **Login/bootstrap:** trusted-dev `POST /api/v1/session` with `{issuer, external_subject, display_name?}`. `201` `{session,principal,csrf_token}` plus `ff_session` / `ff_csrf`. Local/dev only — never rewrite login. Production identity is `POST /embed/exchange`. First-run wizard chrome (B.6 / #339 — **keep #339 open**) gates the standalone shell on `GET /api/v1/bootstrap`: `200` + `incomplete` → wizard; `200` + `complete` or `401` → product home (`/workflows`). Never on `/embed/v1` (ADV-021 `session.embed` only). Steps stay fail-closed: `POST /api/v1/bootstrap/persistence` `{confirm:true}` → `POST /api/v1/bootstrap/admins` `{issuer, external_subject, display_name?}` → `POST /api/v1/bootstrap/public-url` `{publicBaseUrl}` → `POST /api/v1/bootstrap/tls` `{action:"create-self-signed"}`, `{action:"upload", certPem, keyPem}`, or `{action:"skip"}` (API landed; Chloe Skip chrome is follow-up). No password field (local login not landed). PEM POST once; never `localStorage` for passwords / PEMs / KEK. Status never echoes the public URL. After complete, Settings (`#bootstrap` / `#persistence` / `#public-url` / `#tls`) links out — do not remount the wizard. Skip is not a permanent lockout. See [first-run bootstrap](../architecture/flowforge-first-run-bootstrap.md).
