...
| Credential vault | `/credentials`, `/new`, `/{id}` | Workbench find (R5.1 / #264 — **keep #264 open**): display-name search + type/status/tag filters, useful columns, open-to-detail. Detail operate (R5.2 / #265 — **keep #265 open**): test / rotate / usage / deletion-impact on existing vault routes; disable/enable stay clear; rotate returns display-name + UUID only. Gracie R5 security line (inherit #266): no KEK in the browser; display-name + UUID only; secrets never in YAML / search / analytics; unexpected plaintext is a contract bug (strip + stop). Dedicated routes remain. Inspector create reuses the masked wizard. |
...
- The workflow editor selects credentials by display name/reference; it does not expose secret values. The `/credentials` workbench (R5.1 / #264 — **keep #264 open**) finds by display name with type/status/tag filters and columns (display name, type, status, tags, last test, rotated, Open). Rows open existing `/credentials/{id}`. Metadata only — no connection strings, keys, tokens, provider internals, or `CREDENTIAL_KEK`.

Operator routes (Chloe, E4.1 + R5.1 + R5.2): `/credentials` (display-name find), `/credentials/new` (wizard), `/credentials/{id}` (operate detail). Contract adapter: `apps/web/src/lib/credential-contract.ts`. Unexpected secret fields on API responses are stripped and treated as a contract bug.
