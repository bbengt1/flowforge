...
3. **Inspector** (remembered-open drawer + persistent satellite): selected **node** focuses a conversation shell — type-specific parameters / `with` (core / Kubernetes / SSH / script / HTTP; not a bare JSON blob) / pins / credential display names (pick or add a vault credential without leaving; secret entry stays in the masked wizard). First paint opens the rail unless the operator hid it (remembered). An **Inspector** satellite stays on the canvas when the drawer is closed. Selected **workflow** shows tabs **Triggers / Versions / Pins**. Selected **edge** explains port compatibility. Validation groups errors and links to a node or YAML path. No expression language. Field-path mapping is R3.2 / #247.
