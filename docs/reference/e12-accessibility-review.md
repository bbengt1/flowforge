# E12.3 accessibility review
 ...
 review of primary operator/admin surfaces in `apps/web` before
...
The API covers API/OpenAPI, deploy, incident, backup, and the threat-model
...
R7.4 rewrite satellites (.
) Inherit `R7_HARD_LINE`. No SR graph rewrite.
...
| Lighthouse CI | Playwright + `@axe-core/playwright` gates serious/critical findings on the primary surfaces (G.3.3) | Lighthouse remains optional; do not add a second a11y gate |
...
copy (see E12.1 [embed-ui-evidence.md](e12-security-evidence/embed-ui-evidence.md)).

## Ownership

- this review, operator/admin UI guide, `apps/web` a11y
 fixes above.
- API/OpenAPI accuracy, deploy/ops/incident/backup docs,
 threat-model review.until both halves land.
