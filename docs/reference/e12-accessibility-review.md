...
Product a11y principles: [Frontend UI](frontend-ui.md#accessibility-and-responsive-behavior).
 owns API/OpenAPI, deploy, incident, backup, and the threat-model
...
- This review, the operator/admin UI guide, and the `apps/web` fixes above.
- API/OpenAPI accuracy, deploy/ops/incident/backup docs, and the threat-model review. Both halves are required before a production sign-off.
