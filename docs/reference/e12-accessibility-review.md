...
| Authoring | `/workflows/{id}` · canvas, YAML, wizard, validation | Canvas `role="application"` + node/edge labels. YAML `htmlFor`. Validation `aria-live`. Wizard dialog + Esc. |
...
| Canvas `role="application"` is a custom widget | Keyboard pan/select/zoom exist; a full screen-reader graph is not implemented | Inspector-first announcements when a node is selected (partially present) |
...
| Touch “inspector-first” editing | Documented in frontend-ui; desktop keyboard is the MVP path | Separate UX story |
