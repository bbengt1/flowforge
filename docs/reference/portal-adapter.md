...
| `POST` | `/api/v1/embed/exchange` | assertion | no | E11.1/E11.2 exchange. Not Portal-specific. Mint writes `ctx=portal` + `host=iss`; exchange selects `PORTAL_*` from that signed claim. Optional `X-FlowForge-Host-Issuer` + `hostContext=portal` must agree. Issues CHIPS cookies (`SameSite=None; Secure; Partitioned`). Bound sessions cannot `POST /tenants` or `POST /workspaces`. Cookie not sent is `401`/`403`. |
...
| Host issuer bind (ADV-023) | Mint writes `ctx=portal` and `host=iss`. Exchange selects `PORTAL_*` from that signed claim. `EmbedExchangeGate` may still send `X-FlowForge-Host-Issuer` from `GET /portal/adapter` `issuers` / server `PORTAL_ISSUER` and `X-FlowForge-Host-Context: portal` as a consistency check (must agree). Never copy `iss` from the assertion. Wrong-issuer-for-host is `403`. Prefer no chrome rewrite beyond those headers (`PORTAL_HOST_ISSUER_RULES`). |
...
- Rewriting `apps/web` product pages (The canvas wires the host using this map)
