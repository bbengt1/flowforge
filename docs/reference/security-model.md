...
| Control plane → worker | Queue data is untrusted transport data. | Sign or authenticate jobs; include workspace, workflow-version digest, policy revision, expiration, lease/fencing data, and correlation ID; re-authorize before execution. `JOB_BINDING_SECRET` and `SCRIPT_SIGNING_KEY` are required at boot (missing or malformed fails closed; no per-process random default). |
