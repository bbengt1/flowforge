...
| 10 | Dependency / image provenance | Scripts + sibling CI | Suite: `check-approved-bases.sh`, `check-deploy-defaults.sh`, `govulncheck-gate.py` fixtures. **Sibling (required on the same PR):** `supply-chain.yml` `govulncheck` + `image-scan` + `web-image-scan` (Trivy fs HIGH/CRITICAL, image CRITICAL, SPDX SBOM, `write-provenance.sh`). Policy: [supply-chain/policy.md](../../deploy/supply-chain/policy.md) |
