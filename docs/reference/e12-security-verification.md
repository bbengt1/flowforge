...
| 10 | Dependency / image provenance | Scripts + sibling CI | Suite: `check-approved-bases.sh`, `check-deploy-defaults.sh`, `check-image-signing.sh`, `govulncheck-gate.py` fixtures. **Sibling (required on the same PR):** `supply-chain.yml` `govulncheck` + `image-scan` + `web-image-scan` (Trivy fs and image HIGH/CRITICAL with `--ignore-unfixed`, SPDX SBOM). `publish-images` signs and attests on `main` only. Policy: [supply-chain/policy.md](../../deploy/supply-chain/policy.md) |
...
- Harness, API/engine negatives, the CI gate, and this map are the control-plane evidence.
- UI and embed rows marked **Done** are browser evidence. Optional rows stay open when a later pass adds screenshots.
