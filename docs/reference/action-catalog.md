...
This catalog distinguishes **core** nodes (first implementation target), **next** nodes (planned after execution safety is proven), and **provider** nodes (connector-specific work). A visible catalog entry does not imply access: the UI lists only actions enabled for the workspace and principal, and the API independently authorizes every request and dispatch. **A catalog entry also does not imply runtime execution.** Compose’s local worker evaluates six core nodes only; Kubernetes, SSH, script, and HTTP nodes fail closed until a production worker ships ([Implemented vs Specified](../architecture/implemented-vs-specified.md)).
