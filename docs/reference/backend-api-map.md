...
| `POST /api/v1/bootstrap/tls` | Wizard step 4 (B.5 / #338). Enables TLS via `{action:"create-self-signed"}` or `{action:"upload", certPem, keyPem}` (PEM POST once). Writes the pair to process `TLS_CERT_FILE` / `TLS_KEY_FILE` (`internal/tlsmaterial`, 0600). Sets `steps.tls.ready=true` and `steps.tls.mode` (`self_signed` \| `uploaded`) via `Store.SetTLS`, then **`Store.MarkComplete`** so the wizard never reappears. **Never returns** the key, PEM, or KEK. ACME / Let’s Encrypt is out of scope (`400`). Incomplete installs may call this **without** a session (same openness as B.1–B.4). Public URL must be ready first (`409`). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present; unauthenticated incomplete POSTs have no session, so CSRF does not apply. Missing/unwritable TLS paths → `503`. Settings later reads `GET /bootstrap` `steps.tls.mode` only. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/tls`. | `200` `BootstrapStatus` (`steps.tls.ready=true`, `steps.tls.mode`, `complete=true`) | `400` `403` `409` (already complete, or publicUrl not ready) `503` (TLS files / store down) |
