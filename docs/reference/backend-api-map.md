...
| `POST /api/v1/bootstrap/admins` | Wizard step 2 (B.3 / #336). Upserts `issuer` + `external_subject` (optional `display_name`) via `localseed.ProvisionAdmin` / `identity.UpsertUser`, grants workspace admin on the default localseed tenant/workbench, then sets `steps.firstAdmin.ready=true` via `bootstrap.Store.SetStep`. Does **not** mark `complete`. Local login has not landed: a non-empty `password` is `400` and is never stored or echoed. Incomplete installs may call this **without** a session (same openness as B.1/B.2). Persistence must be ready first (`409`). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present; unauthenticated incomplete POSTs have no session, so CSRF does not apply. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/admins`. | `201` `BootstrapStatus` (`steps.firstAdmin.ready=true`, `complete=false`) | `400` `403` `409` (already complete, or persistence not ready) `503` (store down) |
