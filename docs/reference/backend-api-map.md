...
**E12.2:** no operator UI for connection headroom, queue lag, or storage growth. Those stay [e12-resilience-capacity.md](e12-resilience-capacity.md) + `scripts/e12-resilience-suite.sh`.## Browser sessions (E2.3)
...
**ADV-021 retarget map:** Drive embed chrome from `GET /session` `session.embed` — not assertion leftovers, catalog guesses, or host query. Adapter: `parseEmbedChromeFromSession` / `EMBED_CHROME_FROM_SESSION` in `apps/web/src/lib/embed-contract.ts`. Prefer **no product-shell rewrite in this API story**;ADV-024:** Hide membership/isolation catalog chrome unless `session.embed.capabilities` includes `workspace.administer` or `platform.administer` (or catalog `rules.membershipIsolationGranted`). Parser: `parseCatalogMembershipIsolationGranted` / `catalogRoutesForGrant` / `EMBED_CATALOG_MEMBERSHIP_ISOLATION`. Do not treat catalog `capabilities` or ungranted `routes[]` as nav authority. `frameAncestors` stays public..
