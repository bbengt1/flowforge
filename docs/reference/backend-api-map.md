...
| `POST /api/v1/embed/exchange` | Refresh overlap from the store, refuse expired/retired `overlapUntil` kids, then validate iss (merged embed+portal allowlist; empty fails closed)/aud/nbf/exp/jti/capabilities/workspace, atomically consume `jti`, bind tenancy onto `ff_session` with CHIPS cookies (`SameSite=None; Secure; Partitioned`). Body `{assertion,sdk?}`. No CSRF. Bound sessions cannot `POST /tenants` or `POST /workspaces`. | `201` `{session,principal,csrf_token,assertion,workspace,tenant,capabilities}` (`session.embed` present; Set-Cookie is Partitioned) | `400` missing claims `401` audience/expired/nbf/signature/unknown or expired-overlap kid **or cookie not sent later** `403` empty/unknown issuer or tenancy **or CSRF cookie missing in iframe** `409` replay `503` |
