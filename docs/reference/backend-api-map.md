...
| `POST /api/v1/oidc/callback` | Body `{code, state}` plus the matching `ff_oidc_state` cookie. Mints the same `ff_session` / `ff_csrf` pair as Login. Cookie cleared either way. No tokens or secrets in the JSON. Bad or replayed state/code is `401` `Invalid credentials.` | `201` `{session,principal,csrf_token}` | `400` `401` `429` `503` |
