...
**V.0a / Chloe:** `POST /login` may return the same `429` `rate-limited` with `Retry-After`. Treat as backoff only — do not treat 429 as invalid credentials. Env: `LOGIN_RATE_LIMIT_IP` (60), `LOGIN_RATE_LIMIT_IDENTIFIER` (30), `LOGIN_RATE_LIMIT_WINDOW` (`1m`). These counters are separate from embed exchange.
