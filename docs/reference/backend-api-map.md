...
| `POST /api/v1/approvals/{approvalId}/decide` | Fresh auth. `{decision, note?}`. No self-approval. A closed approval, one whose run is canceled, failed, or deleted, or one whose gate is no longer `waiting`, is `409` `approval_closed`. A still-pending row whose gate has left `waiting` is closed as `expired` in that transaction. Nothing is recorded as approved. | `200` | `400` `401` `403` `404` `409` (expired/invalidated/not pending/`approval_closed`) |
