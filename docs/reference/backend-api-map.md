...
| `GET /api/v1/metrics` | Baseline Prometheus text metrics (request count and duration). Requires authenticated principal + `platform.administer` (`PLATFORM_ADMINS`) or a machine grant of `ops.metrics.read`. When `MACHINE_REQUIRE` includes `metrics`, a missing/revoked/ungranted principal is `503`. | `200` `text/plain` | `401` `403` `503` |
