...
Foundation responses also set restrictive content, referrer, and permissions policies. Structured logs are JSON and secret-free: they record method, path, route, status, duration, bytes, and `request_id`, and never record `Authorization`, cookies, query strings, or request bodies. Metrics labels are method, route, and status only.
