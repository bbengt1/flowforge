...
Statuses: `pending`, `approved`, `rejected`, `expired`, `invalidated`, `canceled`. `canceled` is set when the run is canceled (`closeReason: run_canceled`), stopped because the workflow was deleted (`closeReason: workflow_deleted`), or the pinned requirement cannot be rebuilt (`closeReason: requirement_unresolvable`). No decider is recorded. That close does not resume the gate on `expired`. The gate step and job fail with error code `requirement_unresolvable`, and the run follows the normal failed-step roll-up in that same transaction. A sibling job already queued, claimed, or running finishes first; the run then ends `failed` with `requirement_unresolvable`, which frees the concurrency slot. Downstream of the failed gate does not run. `requirement_unresolvable` means the version lookup, the policy evaluation, or the matching requirement is missing. A missing or deleted workflow still fails the run with `workflow_deleted`. Closed rows leave `GET /approvals?status=pending`. That filter also hides a still-`pending` row whose `expires_at` is not in the future, and a pending row whose stored role or target the caller cannot decide. Binding fields on every requirement/record: `workflowVersionId`, `workflowDigest`, `targetId`/`targetVersionId`/`targetDigest`, `policyResourceId`/`policyVersionId`/`policyDigest`/`policyRevision`, `operation`, `nodeId`, `expiresAt`, `bindingFingerprint`.
...
| `requirement_unresolvable` | run `failed` | Not an HTTP problem code. A pending approval whose pinned requirement cannot be rebuilt is `canceled` with `closeReason` `requirement_unresolvable` and no decider. The waiting gate step and its job fail with that error code. The run follows the normal failed-step roll-up: a sibling already queued, claimed, or running finishes, then the run ends `failed` with that reason, which frees the concurrency slot. Downstream steps do not run, including a step wired to `expired`. This code means the version lookup, the policy evaluation, or the matching requirement is missing. A missing or deleted workflow still fails the run with `workflow_deleted`. |
