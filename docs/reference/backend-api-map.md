...
**ADV-024 / Chloe:** Hide membership/isolation catalog chrome unless `session.embed.capabilities` includes `workspace.administer` or `platform.administer` (or catalog `rules.membershipIsolationGranted`). Parser: `parseCatalogMembershipIsolationGranted` / `catalogRoutesForGrant` / `EMBED_CATALOG_MEMBERSHIP_ISOLATION`. Do not treat catalog `capabilities` or ungranted `routes[]` as nav authority. `frameAncestors` stays public. Relates to #153 / Part of #130 — **Keep #153 open**.
...
**D1 canvas layout (issue #238, Chloe UI next):** additive optional `metadata.ui.layout` on `flowforge/v1` (`version: 1`, `nodes.<id>: {x,y}` finite numbers). API stores and returns it on draft save/load and versions. Extra node keys are stripped; non-object / non-finite layout is treated as absent (auto-layout). Layout never carries edges, types, `with`, credentials, or ports and never invents a graph. **Executor, `POST /policy/evaluate`, port typing, and dispatch ignore it.** Embed and standalone share this API. Do not close #238 until Chloe wires the canvas.
