...
| --- | --- | --- | --- |
| `GET /api/v1/health` | Process liveness; no dependency check. Unauthenticated for Kubernetes probes. Publishes non-secret `version` / `sha` from image ldflags or `BUILD_VERSION` / `BUILD_SHA`. | `200 {"status":"ok","version":"dev","sha":"unknown"}` (identity may be a real tag/SHA) | — (missing identity stays `dev`/`unknown`; never 5xx) |
| `GET /api/v1/readiness` | PostgreSQL dependency readiness. Unauthenticated for Kubernetes probes. Ready body includes the same `version` / `sha` as `/health`. | `200 {"status":"ready","version":"…","sha":"…"}` | `503` RFC 9457 Problem Details (`dependency-unavailable`) — no version fields |
| `GET /api/v1/bootstrap` | Standalone first-run wizard gate (B.1). Status flags only — no secrets, KEK, password hashes, private keys, or the stored public URL. Incomplete installs may call this **without** a session so the wizard can start before any admin exists. After `complete`, normal product auth (cookie session or trusted-dev headers) is required (`401`). **Never use this to gate `/embed/v1`.** Embed catalog/exchange/session stay independent. Trusted-dev / compose localseed that already created admin + URL marks `complete=true` / `incomplete=false` / `skipped=true`. Contract map: [first-run bootstrap](../architecture/flowforge-first-run-bootstrap.md). Same-origin proxy: `GET /api/control-plane/bootstrap`. | `200` `{complete,incomplete,skipped,standaloneOnly,steps}` | `401` (complete + no session) `503` |
| `POST /api/v1/bootstrap/persistence` | Wizard step 1 (B.2). Confirms process `DATABASE_URL` PostgreSQL is reachable, then sets `steps.persistence.ready=true` via `bootstrap.Store.SetStep`. Does **not** mark `complete`. Body is `{confirm:true}` only — never a DSN, password, or `DATABASE_URL`. Incomplete installs may call this **without** a session (same openness as B.1 GET). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/persistence`. | `200` `BootstrapStatus` (`steps.persistence.ready=true`, `complete=false`) | `400` `403` `409` (already complete) `503` (PostgreSQL / store down) |
| `POST /api/v1/bootstrap/admins` | Wizard step 2 (B.3). Upserts `issuer` + `external_subject` (optional `display_name`) via `localseed.ProvisionAdmin` / `identity.UpsertUser`, create-or-binds workspace admin on the default localseed tenant/workbench (`local` / `default`, created if missing so path-2 `SEED_LOCAL_DEFAULTS=0` still has a selectable workbench after `POST /login`), then sets `steps.firstAdmin.ready=true` via `bootstrap.Store.SetStep`. Does **not** mark `complete`. Optional `password` is POSTed once, stored only as a bcrypt hash on `local_logins` (identifier = `external_subject`) for `POST /login`, and is **never echoed**. Omit password to skip a local-login credential. Incomplete installs may call this **without** a session (same openness as B.1/B.2). Persistence must be ready first (`409`). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present; unauthenticated incomplete POSTs have no session, so CSRF does not apply. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/admins`. | `201` `BootstrapStatus` (`steps.firstAdmin.ready=true`, `complete=false`) | `400` `403` `409` (already complete, or persistence not ready) `503` (store down) |
| `POST /api/v1/bootstrap/public-url` | Wizard step 3 (B.4). Persists `{publicBaseUrl}` on `instance_bootstrap.public_base_url` via `bootstrap.Store.SetPublicURL` and sets `steps.publicUrl.ready=true`. Does **not** mark `complete`. HTTPS preferred; HTTP allowed for local (e.g. `http://localhost:3000`). Origin only — no userinfo, query, fragment, or path. **Never echoes** the URL on this response or `GET /bootstrap` (Settings read later). Incomplete installs may call this **without** a session (same openness as B.1–B.3). First admin must be ready first (`409`). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present; unauthenticated incomplete POSTs have no session, so CSRF does not apply. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/public-url`. | `200` `BootstrapStatus` (`steps.publicUrl.ready=true`, `complete=false`) | `400` `403` `409` (already complete, or firstAdmin not ready) `503` (store down) |
| `POST /api/v1/bootstrap/tls` | Wizard step 4 (B.5 /, B.7). Enables TLS via `{action:"create-self-signed"}`, `{action:"upload", certPem, keyPem}` (PEM POST once), or `{action:"skip"}`. Create/upload write the pair to process `TLS_CERT_FILE` / `TLS_KEY_FILE` (`internal/tlsmaterial`, 0600). Skip writes **no** PEM/key and does not require those paths. Sets `steps.tls.ready=true` and `steps.tls.mode` (`self_signed` \| `uploaded` \| `skipped`) via `Store.SetTLS`, then **`Store.MarkComplete`** so the wizard never reappears. Skip is not a permanent lockout — Settings `#tls` can enable create/upload later. **Never returns** the key, PEM, or KEK. ACME / Let’s Encrypt is out of scope (`400`). Incomplete installs may call this **without** a session (same openness as B.1–B.4). Public URL must be ready first (`409`). After `complete`, wizard handler is **`409 Conflict`** (Settings-only). Embed sessions are `403`. CSRF required when `ff_session` is present; unauthenticated incomplete POSTs have no session, so CSRF does not apply. Missing/unwritable TLS paths → `503` on create/upload only. Settings later reads `GET /bootstrap` `steps.tls.mode` only. **Never use from `/embed/v1`.** Same-origin proxy: `POST /api/control-plane/bootstrap/tls`. | `200` `BootstrapStatus` (`steps.tls.ready=true`, `steps.tls.mode`, `complete=true`) | `400` `403` `409` (already complete, or publicUrl not ready) `503` (TLS files on create/upload / store down) |
| `GET /api/v1/metrics` | Prometheus text: HTTP request count/duration plus OpenTelemetry business metrics (queue depth, queue lag, lease claims, lease expirations, execution outcomes, vault ops). Labels are closed enums. Requires authenticated principal + `platform.administer` (`PLATFORM_ADMINS`) or a machine grant of `ops.metrics.read`. When `MACHINE_REQUIRE` includes `metrics`, a missing/revoked/ungranted principal is `503`. | `200` `text/plain` | `401` `403` `503` |
...
**ADV-020 / scrapers:** Prometheus and other scrapers authenticate with an existing session. There is no anonymous scrape token. `ops.metrics.read` is an explicit machine grant (platform-scoped, not a workspace role). Fail closed.

| Method | How |
| --- | --- |
| `POST /api/v1/machine/token` then `Authorization: Bearer <token>` | Opaque `ff_session` for a machine principal (`client_id` + secret or signed assertion). Preferred for in-cluster scrapers. Grant `ops.metrics.read` (or explicit `platform.administer`). |
| `Authorization: Bearer <token>` | Same cookie token for a `PLATFORM_ADMINS` principal. |
...
| Trusted-dev headers | `X-FlowForge-Issuer` / `X-FlowForge-Subject` matching `PLATFORM_ADMINS`, only when `TRUSTED_DEV_IDENTITY_HEADERS` is on in non-production. Production is `401`. Not the machine identity. |
...
Empty `PLATFORM_ADMINS` is `403` even with a valid session. Workspace `admin` is not enough. `GET /health` and `GET /readiness` stay open so kubelet probes do not need credentials. There are no `/healthz` / `/readyz` aliases.
 ...
 no product UI. Home/shell OpenAPI links will `401`/`403` for callers who are not platform-admins; do not add a metrics or swagger screen.

**E12.2 / ** no operator UI for connection headroom, queue lag, or storage growth. Those stay [e12-resilience-capacity.md](e12-resilience-capacity.md) + `scripts/e12-resilience-suite.sh`.

## Browser sessions (E2.3)

Browser clients use cookie sessions from standalone `POST /login` (local email/username + password), OIDC Authorization Code + PKCE (`POST /oidc/start`, `POST /oidc/callback`), or `POST /embed/exchange`. OIDC is opt-in and fail-closed when unset. Production does **not** treat `X-FlowForge-Issuer` / `X-FlowForge-Subject` as authentication and `POST /session` must not upsert principals from those values. Self-asserted header identity exists only behind `TRUSTED_DEV_IDENTITY_HEADERS` plus an explicit non-production `APP_ENV` (see [security model](security-model.md) and [deployment](../deployment.md)). When `ff_session` is present, identity comes only from the session; conflicting identity headers fail closed (`403`). Header-only callers skip CSRF, and only when trusted-dev is on. Never rewrite product login onto trusted-dev `POST /session`.

**UI route map:** call the API origin with `credentials: "include"`. Do not store the session token or CSRF secret in `localStorage`. Read `csrf_token` from the JSON body (or the `ff_csrf` cookie) and send it as `X-CSRF-Token` on every state-changing request.

| Cookie | Flags | Purpose |
| --- | --- | --- |
| `ff_session` (top-level / `POST /login` or trusted-dev `POST /session`) | `HttpOnly`, `SameSite=Lax`, `Secure` on HTTPS, `Path=/api/v1` | Opaque session id (server stores SHA-256 only) |
| `ff_csrf` (top-level / `POST /login` or trusted-dev `POST /session`) | readable, `SameSite=Strict`, `Secure` on HTTPS, `Path=/api/v1` | Double-submit CSRF pair with `X-CSRF-Token` |
...
Idle default **30m**, absolute default **12h** (`SESSION_IDLE_TIMEOUT` / `SESSION_ABSOLUTE_TIMEOUT`). Refresh extends idle only; it cannot pass the absolute cap. Stale, revoked, or forged cookies are `401`. **ADV-019:** `DELETE /workspace` (soft-disable) revokes every embed session bound to that workspace_id or `(tenant_id, workbench_key)` — including CHIPS cookies from `POST /embed/exchange`. Later requests with those cookies are `401`. Unbound standalone sessions and sessions bound to other workspaces stay valid. If revoke cannot complete, the workspace is not deleted (fail closed). If disable fails after revoke, sessions stay revoked (safe). A PostgreSQL trigger also revokes on `status=disabled` and on hard `DELETE` of the workspace row.
...
| Route | Purpose | Success | Failure |
| --- | --- | --- | --- |
| `POST /api/v1/machine/token` | Machine principal session. Body `{client_id, secret}` or `{client_id, assertion}`. Mints the same standalone `ff_session` / `ff_csrf` as Login (`session.embed` omitted). Not Login, not trusted-dev `POST /session`, not `POST /embed/exchange`. Unknown client and bad secret are the same `401`. Secret and assertion are never echoed. Rate-limited separately (IP 60/min, client 30/min). | `201` session | `400` `401` `429` `503` |
| `GET /api/v1/machine/principals` | List machine principals. `platform.administer` + CSRF. Display name, UUID, `client_id`, status, grants only. | `200` `{items}` | `401` `403` `503` |
| `POST /api/v1/machine/principals` | Create a principal. `secret` or `assertion_public_key` POST once, never echoed. Grants explicit (empty authorizes nothing). `embed.impersonate` rejected. Workspace-scoped grants need a workspace binding. | `201` principal view | `400` `401` `403` `409` `503` |
| `GET /api/v1/machine/principals/{id}` | One principal. Same view. No secret material. | `200` | `401` `403` `404` `503` |
| `POST /api/v1/machine/principals/{id}/rotate` | Replace secret and/or public key. Grants unchanged. New secret not echoed. | `200` | `400` `401` `403` `404` `409` `503` |
| `POST /api/v1/machine/principals/{id}/revoke` | Disable the user and revoke live sessions. | `200` | `401` `403` `404` `409` `503` |
| `POST /api/v1/login` | V.0a standalone local login. Body `{identifier\|email\|username, password}`. Verifies a `local_logins` hash (identifier is case-insensitive `external_subject` from B.3, or first-run `admin`) and mints the same first-party `ff_session` / `ff_csrf` pair (Lax / Strict, `Path=/api/v1`). No CSRF. Password POST once; never echoed. Unknown identifier and wrong password are the same `401` without saying which field failed. First-run one-time `admin` / `admin` sets `session.must_change_password` so can gate product chrome. Rate-limited by client IP (default 60/min) and identifier (default 30/min) **before** lookup/bcrypt; burst is `429` `rate-limited` with `Retry-After`. Separate from embed exchange counters. Store failures other than unknown identifier are `503`. A durable `auth_lockouts` row (`LOCKOUT_MAX_FAILURES`, default 5) is the same `401` after a correct password and survives process restart. `GET /session` after success omits `session.embed`. **Never use from `/embed/v1`.** Embed stays `POST /embed/exchange`. This body is not an IdP start or callback. Same-origin proxy: `POST /api/control-plane/login`. | `201` `{session,principal,csrf_token}` | `400` `401` `429` `503` |
| `POST /api/v1/oidc/start` | OIDC Authorization Code + PKCE start. Empty body or `{}`. Returns `{authorization_url, state, expires_at}` and sets `ff_oidc_state` (`HttpOnly`, `SameSite=Lax`). No `client_secret` and no `code_verifier` in the URL or JSON. Unset OIDC is `503` `OIDC is not configured.` IdP discovery failure is `503`. Rate-limited 30/min per IP. | `200` | `400` `429` `503` |
| `POST /api/v1/oidc/callback` | Body `{code, state}` plus the matching `ff_oidc_state` cookie. Mints the same `ff_session` / `ff_csrf` pair as Login. Cookie cleared either way. No tokens or secrets in the JSON. Bad or replayed state/code, and a locked `auth_lockouts` row, are `401` `Invalid credentials.` A successful unlocked callback clears the counter. | `201` `{session,principal,csrf_token}` | `400` `401` `429` `503` |
| `GET /api/v1/session/mfa` | TOTP status. No secret. `applicable` is true only for local-login and OIDC. Machine, trusted-dev, and embed report `applicable: false`, `satisfied: true`. | `200` `{method,enrolled,satisfied,applicable,privileged_permissions}` | `401` `503` |
| `POST /api/v1/session/mfa/enroll` | CSRF. Empty body. Returns status plus `otpauth_uri` once (contains the TOTP secret — display once; do not store). Confirmed factor is `409` with no URI. Missing `MFA_SECRET_KEY` is `503`. | `200` | `401` `403` `409` `503` |
| `POST /api/v1/session/mfa/verify` | CSRF. Body `{code}`. Steps up this session. No code echo. Bad or replayed code is `401`. 8/min per user. Privileged `platform.administer` and `credential.*` on local-login/OIDC without enroll or verify are `403` `mfa-required`. | `200` | `400` `401` `429` `503` |
| `GET /api/v1/users/{userID}/lockout` | Durable lock state. `platform.administer`. Embed sessions are `403`. JSON is `user_id`, `locked`, `failed_count`, optional `locked_at`. No hash or password. | `200` | `401` `403` `503` |
| `POST /api/v1/users/{userID}/unlock` | CSRF. Deletes the `auth_lockouts` row. Does not re-enable a disabled user. Same secret-free JSON with `locked: false`. | `200` | `401` `403` `503` |
| `GET /scim/v2/ServiceProviderConfig` | SCIM discovery. `Authorization: Bearer` `SCIM_BEARER_TOKEN` only. Unset SCIM is `503` `SCIM is not configured.` Wrong or missing bearer is `401` with `WWW-Authenticate: Bearer`. Query `access_token` is `401` and is not echoed. `Content-Type: application/scim+json`. | `200` | `401` `503` |
| `GET /scim/v2/Schemas` | SCIM schema discovery. Same bearer. | `200` | `401` `503` |
| `GET /scim/v2/ResourceTypes` | SCIM resource-type discovery. Same bearer. | `200` | `401` `503` |
| `GET /scim/v2/Users` | List active SCIM users. Filter `userName`, `externalId`, or `id` with `attr eq "value"`. `startIndex` is 1-based; `count` default 100, max 100. Other query keys are `401`. | `200` | `400` `401` `503` |
| `POST /scim/v2/Users` | Provision. `userName` required. `externalId` (else `userName`) is `users.external_subject` under `SCIM_ISSUER` (or `OIDC_ISSUER` when omitted; both set must match). IdP mapping: set `externalId` to the OIDC `sub`. Password and secret-like keys are `400` and are not echoed. Re-POST of a deprovisioned name reactivates. | `201` | `400` `401` `409` `503` |
| `GET /scim/v2/Users/{id}` | One active user. Deprovisioned is `404`. | `200` | `401` `404` `503` |
| `PUT /scim/v2/Users/{id}` | Replace supported attributes (`userName`, `externalId`, `displayName`, `name.formatted`, `active`). `externalId` that would change `external_subject` is `400`. | `200` | `400` `401` `404` `409` `503` |
| `PATCH /scim/v2/Users/{id}` | `add`/`replace` on the same attributes. `active: false` disables the user, revokes sessions, and removes workspace memberships except the last admin. | `200` | `400` `401` `404` `409` `503` |
| `DELETE /scim/v2/Users/{id}` | Deprovision: same disable as `active: false`, plus `deprovisioned_at` so later GET is `404`. | `204` | `401` `404` `503` |
| `GET /scim/v2/Groups` | List active workspaces as Groups (`id` = workspace UUID, `displayName` = name, `externalId` = workbench key). Filter `displayName` or `id`. | `200` | `400` `401` `503` |
| `POST /scim/v2/Groups` | Rejected. Workspaces are not created through SCIM. | — | `400` `401` `503` |
| `GET /scim/v2/Groups/{id}` | One workspace and its members (`value` + `display` only). | `200` | `401` `404` `503` |
| `PUT /scim/v2/Groups/{id}` | Rejected. | — | `400` `401` `503` |
| `PATCH /scim/v2/Groups/{id}` | `add`/`remove` `members` only. Add grants `SCIM_DEFAULT_ROLE` (default `viewer`) and does not strip other roles. Remove of the last admin is `409` and leaves the binding. Member ids are not echoed in errors. | `200` | `400` `401` `404` `409` `503` |
| `DELETE /scim/v2/Groups/{id}` | Rejected. Workspaces are not deleted through SCIM. | — | `400` `401` `503` |
| `POST /api/v1/session` | Trusted-dev only: create session from identity headers and/or JSON `{issuer,external_subject,display_name?}`. Headers win; a conflicting body is `403`. Production (fail-closed) returns `401` and does not upsert a principal. Never rewrite login onto this route. Prefer `POST /login` (standalone) or `POST /embed/exchange` (embed). Sets both cookies when allowed. | `201` `{session,principal,csrf_token}` | `401` `403` (hostile origin, identity conflict, or fail-closed) |
| `GET /api/v1/session` | Current browser session. Cookie required; header-only is `401`. Embed-bound sessions include `session.embed` chrome fields (`mode`, `sdk`, tenant/workbench/workspace identity, capped `capabilities`). Standalone omits `session.embed`. `session.must_change_password` is true while the local-login credential still requires rotation (first-run one-time). No secrets, no raw assertion, no password/hash. | `200` `{session,principal,csrf_token}` | `401` `403` |
...
| `POST /api/v1/session/password` | Change the local-login password (``). Body `{password\|new_password}`. CSRF required. Embed sessions are `403`. New password must meet min length, must not match the current secret, and must not be the one-time default `admin`. Success replaces the hash, clears `must_change_password`, and makes `admin`/`admin` the same `401` as unknown. Password POST once; never echoed. Same-origin proxy: `POST /api/control-plane/session/password`. | `200` `{session,principal,csrf_token}` | `400` `401` `403` `404` `503` |
...
| `POST /api/v1/embed/exchange` | Refresh overlap from the store, refuse expired/retired `overlapUntil` kids, then **verify** iss (signed `ctx` path allowlist + `host=iss`; empty fails closed)/aud/nbf/exp/jti/signature/capabilities **before any workspace lookup**. Mint writes `host=iss` and `ctx=embed\|portal`. Exchange selects `PORTAL_*` vs `EMBED_*` from that signed `ctx` — not from an unauthenticated header. Optional `X-FlowForge-Host-Issuer` / `hostIssuer` and `X-FlowForge-Host-Context` / `hostContext` must agree with those signed claims when sent. `nbf` clock-skew is **30s** default (`EMBED_NBF_LEEWAY`, hard max **60s**); `exp` has no leeway. Durable `jti` consume is one `INSERT … ON CONFLICT DO NOTHING RETURNING` after verify success. Used ids stay reserved 24h past `exp` (`retain_until`). Then resolve `(tenant_id, workbench_key)` and bind tenancy onto `ff_session` with CHIPS cookies (`SameSite=None; Secure; Partitioned`). Body `{assertion,sdk?,hostIssuer?,hostContext?}`. No CSRF. Bound sessions cannot `POST /tenants` or `POST /workspaces`. Invalid assertions do not probe tenant existence. Rate-limited by IP (default 120/min) and issuer\|subject (default 30/min) **before** verify. Authz decisions emit secret-free `embed_audit` events. | `201` `{session,principal,csrf_token,assertion,workspace,tenant,capabilities}` (`session.embed` present; Set-Cookie is Partitioned) | `400` missing claims `401` audience/expired/nbf-beyond-leeway/signature/unknown or expired-overlap kid **or cookie not sent later** (same class if the claimed workspace is missing) `403` empty/unknown issuer, wrong-issuer-for-host, or tenancy **or CSRF cookie missing in iframe** `404` verified assertion for unknown workspace `409` replay `429` `rate-limited` `503` |
...
**ADV-012 / ** `POST /embed/exchange` may return `429` `rate-limited` with `Retry-After`. Treat as backoff only — prefer **no UI change** beyond that. Do not treat 429 as forbidden. Audit payloads never include the compact JWS, signing keys, or session secrets. Env: `EMBED_EXCHANGE_RATE_LIMIT_IP` (120), `EMBED_EXCHANGE_RATE_LIMIT_PRINCIPAL` (30), `EMBED_MINT_RATE_LIMIT_PRINCIPAL` (60), `EMBED_RATE_LIMIT_WINDOW` (`1m`). With PostgreSQL the window is shared across API replicas. It is not the workspace `QUOTA_*` bucket.

**V.0a / ** `POST /login` may return the same `429` `rate-limited` with `Retry-After`. Treat as backoff only — do not treat 429 as invalid credentials. Env: `LOGIN_RATE_LIMIT_IP` (60), `LOGIN_RATE_LIMIT_IDENTIFIER` (30), `LOGIN_RATE_LIMIT_WINDOW` (`1m`). These counters are separate from embed exchange and from workspace quotas. A full workspace quota does not change login. Store failure is `503` `dependency-unavailable`, not `429`.

**G.2.8 ** Mutating routes, expensive lists, artifact downloads, and execution start may return `429` `rate-limited` with `Retry-After` when the workspace token bucket is empty. Execution start may also return `429` when the workspace already has `QUOTA_EXECUTE_CONCURRENCY` open executions (default 20). `GET /health` and `GET /readiness` are not rate limited. No Settings chrome in this change.

**ADV-023 / ** Exchange binds `iss` to the **signed** minting host (`host=iss`, `ctx=embed|portal`). The path allowlist comes from that signed `ctx`, not from a client header. Headers are an optional consistency check: `EmbedExchangeGate` may send `X-FlowForge-Host-Issuer` from `GET /embed/catalog` `issuers` / server `EMBED_ISSUER` (standalone) or `GET /portal/adapter` `issuers` / server `PORTAL_ISSUER` (Portal-framed) plus `X-FlowForge-Host-Context` — never a value peeked from the assertion. A disagreeing header is `403`. Helpers: `bindEmbedExchangeHost`, `resolveEmbedHostBinding`, `exchangeHostBindingFromGate`.

**ADV-017 / ** Assertion `nbf` leeway is API verify only (default 30s, `EMBED_NBF_LEEWAY`, hard max 60s). Future `nbf` beyond leeway is `401`. **No UI change.**

**ADV-018 / ** Production issuers must be `https://` (boot-fail on allowlist load; mint/exchange `403`). **No UI change.**

**ADV-019 / ** After `DELETE /workspace`, cookies for embed sessions bound to that workspace (including CHIPS) are revoked. Later calls are `401` `unauthenticated`. Prefer **no UI beyond existing session-expired / 401 handling**. Unrelated workspace sessions and unbound standalone sessions stay valid. Do not invent a dedicated “workspace deleted” chrome.

**ADV-021 / retarget map:** Drive embed chrome from `GET /session` `session.embed` — not assertion leftovers, catalog guesses, or host query. Adapter: `parseEmbedChromeFromSession` / `EMBED_CHROME_FROM_SESSION` in `apps/web/src/lib/embed-contract.ts`. Prefer **no product-shell rewrite in this API story**;

**ADV-024 / ** Hide membership/isolation catalog chrome unless `session.embed.capabilities` includes `workspace.administer` or `platform.administer` (or catalog `rules.membershipIsolationGranted`). Parser: `parseCatalogMembershipIsolationGranted` / `catalogRoutesForGrant` / `EMBED_CATALOG_MEMBERSHIP_ISOLATION`. Do not treat catalog `capabilities` or ungranted `routes[]` as nav authority. `frameAncestors` stays public.
...
Epic negatives (fail closed): hostile host issuer, replayed assertion (`409` on exchange), cross-tenant/workbench (`403`), no credential plaintext or raw runner-log exposure, embed session tenant/workspace create (`403`), Portal `admin` extra `platform.administer` (`400`).

## Workspace identity and RBAC (E2.1)

In trusted-dev only, identity headers establish the subject for non-browser callers: `X-FlowForge-Issuer` and `X-FlowForge-Subject` (optional `X-FlowForge-Display-Name`). Production requires a cookie session (`POST /login` or embed exchange). Headers never authorize a workspace. The UI session adapter is `apps/web/src/lib/session-contract.ts`.
...
**Key source (no hard-coded secrets):** non-production `CREDENTIAL_KEK` (32-byte AES-256 as standard/raw-URL base64 or 64 hex chars) or `CREDENTIAL_KEK_FILE`. Optional `CREDENTIAL_KEK_ID` is stored as `keyReference` (default `env:CREDENTIAL_KEK` / `file:CREDENTIAL_KEK_FILE`). Production-locked processes refuse that plaintext KEK. They require `KMS_PROVIDER` (`aws`, `gcp`, `azure`, or `vault`) and `CREDENTIAL_KEK_WRAPPED` (KMS ciphertext only). Partial KMS configuration fails closed at boot. Missing/invalid KEK fails closed on create/rotate/test/use (`503` `dependency-unavailable`). Envelope: random DEK + AES-256-GCM payload, DEK wrapped with the data KEK. Online rotation keeps the previous KEK for decrypt while `kek-rotate reencrypt` rewraps DEKs. Operator steps: [KEK rotation](../operations/kek-rotation.md).

**UI route map:** cookie session + `credentials: "include"`; send `X-CSRF-Token` on POST/PATCH/DELETE. JSON is camelCase. Never persist `secret` in `sessionStorage`, `localStorage`, URLs, or analytics. After submit, drop the form values. List/detail/events show only metadata (`displayName`, `type`, `status`, `tags`, `fingerprint`, test/rotation timestamps, `permittedActions`). Host-supplied `id` / `workspace_id` / `workspaceId` on write bodies is `400`. Cross-workspace UUIDs are `404`. Do not rewrite `apps/web` in this API story.
...
3. If `decision=approval-required`, `POST /approvals` `{workflowId, workflowVersionId}` (or let Run 409 after the server materializes pending rows). Inbox: `GET /approvals?status=pending` (also `expires_at > now()`).
...
| `GET /api/v1/approvals` | List. Query `status`, `workflowId`, `workflowVersionId`, `executionId`. `status=pending` also requires `expires_at > now()`. Refreshes stale rows. | `200` `{items}` | `401` `403` |
...
| `POST /api/v1/approvals/{approvalId}/decide` | Fresh auth. `{decision, note?}`. No self-approval. A closed approval, one whose run is canceled, failed, or deleted, or one whose gate is no longer `waiting`, is `409` `approval_closed`. A still-pending row whose gate has left `waiting` is closed as `expired` in that transaction. Nothing is recorded as approved. A database, network, or timeout failure while rebuilding the requirement is `503` `approval_requirement_unavailable` with `Retry-After: 5`. That response records nothing and changes nothing. The problem document has no `errors` array. | `200` | `400` `401` `403` `404` `409` (expired/invalidated/not pending/`approval_closed`) `503` `approval_requirement_unavailable` |
...
Statuses: `pending`, `approved`, `rejected`, `expired`, `invalidated`, `canceled`. `canceled` is set when the run is canceled (`closeReason: run_canceled`), stopped because the workflow was deleted (`closeReason: workflow_deleted`), or the pinned requirement cannot be rebuilt (`closeReason: requirement_unresolvable`). No decider is recorded. That close does not resume the gate on `expired`. The gate step and job fail with error code `requirement_unresolvable`, and the run follows the normal failed-step roll-up in that same transaction. A sibling job already queued, claimed, or running finishes first; the run then ends `failed` with `requirement_unresolvable`, which frees the concurrency slot. Downstream of the failed gate does not run. `requirement_unresolvable` means the pinned version is not found, policy evaluation failed deterministically, or the matching requirement is missing. A database, network, or timeout failure is not this code: decide returns `503` `approval_requirement_unavailable` with `Retry-After: 5`, records nothing, and changes nothing. The problem document has no `errors` array. Resync leaves the approval pending. Compose claim of an unresolvable gate fails the claimed job with this code through the same roll-up, so a later lease expiry does not resume it on `expired`. A transient claim releases the job and sets `available_at` to now plus 30s plus up to 5s of jitter. When a pending approval already exists for the gate, the outer limit is that row's `expires_at`. Otherwise the limit is the gate-ready time plus the parked-approval wait duration: the step `expiresIn`, or `PT1H` when that field is missing or not a duration, and never longer than the existing `P7D` maximum. The gate-ready time is not the job's `available_at` or `updated_at`. Those columns are set when the job leaves `blocked` and then rewritten by claim, release, and recovery. The ready time is the latest usable `finished_at` of satisfied upstream edges in the same execution. Only the final attempt of each upstream node counts: a success uses that attempt's `finished_at`, and a skip counts only when `finished_at` is set and not earlier than the run's `started_at`. A missing or earlier skip stamp is ignored, and a failed earlier attempt is ignored. The ready time is never earlier than `started_at`. A root gate, or a gate whose upstream left no usable settle time, uses `started_at`. `execution_jobs` has no `finished_at`; that step column is the finish time of the upstream job. A missing gate-ready time with no pending approval has no limit. Those timestamps are not rewritten by a transient release, a lease-recovery requeue, or a later claim. The parked approval's `expires_at` is claim time plus that same wait duration. Evaluation still fails a claim that has no `expiresIn`, so the runner does not park that gate. Past the limit the same transaction fails the job and step with `requirement_unresolvable`, cancels a pending approval for that execution and node with `close_reason` `requirement_unresolvable`, and rolls the run up. The gate does not take `expired`. A waiting gate that already has a pending approval and no transient failure still takes the `expired` port at its deadline. `attempt` is unchanged. Other releases are not delayed. A binding correction writes approval event `corrected` with the previous and new approver role and policy pin. Resync rebuilds a pending row from the run's pinned version and that requirement's policy pin, never from current heads, and does not invalidate it at boot. A gate with no policy does not pick up another pin. Decide still checks freshness against current heads. Lease recovery requeues a claimed approval with no pending row and applies that same delay. SQLSTATE 42501 is transient: decide returns 503 `approval_requirement_unavailable` and the row stays pending. A missing or deleted workflow still fails the run with `workflow_deleted`. Closed rows leave `GET /approvals?status=pending`. That filter also hides a still-`pending` row whose `expires_at` is not in the future, and a pending row whose stored role or target the caller cannot decide. Binding fields on every requirement/record: `workflowVersionId`, `workflowDigest`, `targetId`/`targetVersionId`/`targetDigest`, `policyResourceId`/`policyVersionId`/`policyDigest`/`policyRevision`, `operation`, `nodeId`, `expiresAt`, `bindingFingerprint`.
...
**UI route map:** do **not** stack on another feature branch. Cookie session + `credentials: "include"`; send `X-CSRF-Token` on POST. JSON is camelCase. Host-supplied `id` / `workspaceId` is `400`. Do not rewrite `apps/web` in this API story. Wizard/library should read `allowedWith` on the three nodes instead of inferring fields.
...
**UI route map:** do **not** stack on another feature branch. Cookie session + `credentials: "include"`; send `X-CSRF-Token` on POST. JSON is camelCase. Host-supplied `id` / `workspaceId` is `400`. Do not rewrite `apps/web` in this API story. Wizard/library should read `allowedWith` on `ssh.run` and `isolation` / `errors[]` from `GET /ssh/catalog`. until node/config UI surfaces land.
...
Workers claim an E5.2 job, then call `scripts.Execute` after `VerifyForDispatch`. No new browser routes. Read `GET /scripts/catalog` → `isolation` + `errors[]` + `runtimeProfile`. — may land runtime-profile UI separately.
...
Workers still claim an E5.2 job and call `scripts.Execute`. No new browser routes. Read `GET /scripts/catalog` → `io` + `retry` + `errors[]`. — may land I/O schema / result UI separately.
...
Workspace-scoped revoke plus authorized emergency-stop. until revoke/stop UI lands. Do not treat this API story as closing the issue.

**UI route map:** do **not** rewrite `apps/web` in this API story. Cookie session + `credentials: "include"`; send `X-CSRF-Token` on POST. JSON is camelCase. Host-supplied `id` / `workspaceId` is `400`. Cross-workspace UUIDs are `404`. Read `GET /scripts/catalog` → `revocation` + `emergencyStop` + `errors[]`.
...
| `POST /api/v1/workflows/validate` | Parse + graph validation. Body `application/yaml` or JSON `{definitionYaml}`. Requires `workflow.edit`. Optional D1 `metadata.ui.layout` is accepted; `summary.ui.layout` is returned when present. Executor / policy / ports ignore layout. | `200` `{valid,summary,warnings}` | `400` `invalid-workflow` (with `errors`) / `401` `403` `413` |
| `POST /api/v1/workflows/normalize` | Validate, emit deterministic YAML, SHA-256 digest. Same body as validate. Requires `workflow.edit`. Layout is persisted in `definitionYaml` and projected on `summary.ui.layout`. | `200` `{definitionYaml,digest,summary,warnings}` | `400` `invalid-workflow` (with `errors`) / `401` `403` `413` |
...
Digest format: `sha256:<hex>` of normalized YAML. Normalization sorts labels, optional `metadata.ui.layout` node keys, triggers, nodes, edges, and outputs by id/name and emits a single trailing newline. Literal blocks (`manifests`, `source`) keep inner text except trailing-newline normalization.

**D1 canvas layout (issue, UI next):** additive optional `metadata.ui.layout` on `flowforge/v1` (`version: 1`, `nodes.<id>: {x,y}` finite numbers). API stores and returns it on draft save/load and versions. Extra node keys are stripped; non-object / non-finite layout is treated as absent (auto-layout). Layout never carries edges, types, `with`, credentials, or ports and never invents a graph. **Executor, `POST /policy/evaluate`, port typing, and dispatch ignore it.** Embed and standalone share this API. until The canvas wires the canvas.
...
**UI route map:** persist drafts through these shapes. `/workflows` and the Next proxies under `/api/control-plane/workflows/...` are in place. JSON field names are camelCase (`definitionYaml`, `draftRevision`, `workflowVersionId`). Host-supplied `id` / `workspace_id` / `workspaceId` on write bodies is `400`. Cross-workspace UUIDs are `404`. Cookie sessions send `X-CSRF-Token` on POST/PUT.
...
| `POST /api/v1/workflows` | Create workflow + draft revision 1. JSON `{definitionYaml, slug?, name?, folderId?}`. `slug` is optional. Precedence is the JSON `slug`, then `metadata.slug`, then a slug derived from `name` (or `metadata.name`). Derivation lowercases, turns each run of non-letters and non-digits into one hyphen, trims hyphens, prefixes a leading digit with `w-`, caps at 63, and uses `workflow` when nothing remains. Reserved derived words (`catalog`, `validate`, `normalize`) and clashes, including soft-deleted slugs, get a suffix (`-2`, `-3`, …) from the unique index, up to 20 attempts. An explicit clash is not rewritten: a live slug is `409` `conflict` and a tombstone is `409` `workflow_slug_reserved`, both with `errors` path `slug`. The chosen slug is written to `metadata.slug` and stays stable on rename. Missing / null `folderId` is Unfiled. Requires `workflow.edit`. Response `workflow.capabilities.delete` follows the caller. | `201` `{workflow,draft}` | `400` `invalid-workflow` or `invalid-request` (explicit slug) / `401` `403` `404` (folder) `409` `conflict` or `workflow_slug_reserved` |
| `GET /api/v1/workflows/{workflowId}` | Summary including `draftRevision`, `draftDigest`, `folderId`, latest version, and `capabilities.delete`. A caller who can delete also receives `deleteImpact` (`waitingRuns`, `blocked`, `inFlightRuns`), computed by the same classification as delete. Viewers who cannot delete, embed sessions, list, and create omit `deleteImpact`. A tombstone is the same `404` `not-found` as a missing workflow. | `200` workflow | `401` `403` `404` |
| `DELETE /api/v1/workflows/{workflowId}` | Soft-delete. One transaction unpublishes (`status` returns to `draft`), disables triggers and schedules, and sets the tombstone. YAML is not modified. Version history stays stored and becomes unreachable. No purge and no restore. Success is `204` with no body. `workflow.delete` is granted to editors and workspace admins (`workspace.administer`). The workflow owner (`createdBy`) may also delete. `capabilities.delete` is true for those callers and false for a viewer who is not the owner. A viewer who is not the owner gets `403` `forbidden`. The delete handler refuses any embed session with `403` `forbidden` before permission, capability, or ownership checks, including a session whose stored caps still list `workflow.delete`. `capabilities.delete` is false. Ownership does not add edit, publish, trigger, schedule, or other grants beyond the embed caps. `workflow.delete` cannot be minted (rejected, not dropped). The identity proxy refuses embed `DELETE` of this route with `403`; first-party browser `DELETE` stays allowlisted. No `workflow.view` (no membership, other tenant) gets `404` `not-found`, same as an unknown or already-deleted id. A non-terminal run with a job in `queued`, `claimed`, or `running` returns `409` `workflow_has_active_executions` and is not changed. That includes a gate parked while a sibling branch still has a queued, claimed, or running job. Waiting, pending, and blocked work does not block. Delete of a parked run, in that same transaction, cancels those steps and jobs with error code `workflow_deleted`, cancels a `flow.delay` timer, closes pending approvals as `canceled` with `closeReason` `workflow_deleted` and no decider, and fails the run with `workflow_deleted`, which frees the concurrency slot. A later resume, retry, requeue, or claim of a run that was still open fails the run (`failed`, reason `workflow_deleted`) and does not continue it. There is no delete route on `/embed/v1`. | `204` | `401` `403` `404` `409` `workflow_has_active_executions` |
...
Out of scope: `apps/web` rewrite, schedules + durable `flow.approval` wait/resume (E10.3 below), `http.request` / `notification.webhook` / `notification.email` (E10.4).

## Schedules and durable flow.approval (E10.3)

Timezone-explicit, version-pinned schedules plus mid-run `flow.approval` wait/resume that survives worker/pod loss. (enables schedule UI + approval decide UX against this map). Do **not** rewrite `apps/web` in this API story.
...
| `POST /api/v1/approvals/{approvalId}/decide` | Fresh-auth decide **and** resume wait. A deleted workflow stops the run and closes the approval; decide is `409` `approval_closed`. A transient requirement rebuild is `503` `approval_requirement_unavailable` with `Retry-After: 5` and changes nothing. | `200` approval | `401` `403` `409` `approval_closed` `503` `approval_requirement_unavailable` |

Out of scope: `apps/web` rewrite. HTTP and notification action nodes are E10.4 below.

## HTTP and notification actions (E10.4)

Last story on epic. (still has library/wizard UI pending). Do **not** rewrite `apps/web` in this API story.
...
Out of scope: `apps/web` rewrite. No new trigger types.
...
Redacted step state, bounded logs/output, envelope-encrypted artifact metadata, short-lived download grants, retention deletion, and legal hold. Object payloads are encrypted at rest (same `CREDENTIAL_KEK` envelope as the vault) and written to an S3-compatible bucket (`ARTIFACT_S3_*`; optional `ARTIFACT_S3_SSE` of `AES256` or `aws:kms`). Compose uses MinIO. Production-locked processes refuse filesystem and memory stores and do not fall back to a directory when the bucket or credentials are missing. Object keys are `{tenant}/{workspace}/{ref}` UUIDs; `ARTIFACT_S3_PREFIX` is rejected. Draft executions cannot attach run artifacts. Non-production may still use `ARTIFACT_STORE_DIR` or in-process memory when no S3 intent is set. The API **never** returns `storageRef`, DEK/envelope fields, bucket names, or durable public URLs. Credentials are not logged.

**UI route map:** do **not** stack on this feature branch. Paths are intended to be stable on `main`. Cookie session + `credentials: "include"`; send `X-CSRF-Token` on POST. JSON is camelCase. Host-supplied `id` / `workspaceId` / `storageRef` / `url` / `bucket` / `key` on write bodies is `400`. Cross-workspace UUIDs are `404`. Do not rewrite `apps/web` in this API story. Do not call object-store APIs. Do not persist download `href`s. Isolation hook `GET /workspace/artifacts/{id}` is **not** the product artifact API.
...
| `workflow_has_active_executions` | 409 | Soft-delete refused because a non-terminal run has a job in `queued`, `claimed`, or `running`. A gate parked beside that job is included. Waiting, pending, and blocked work does not block. Nothing is changed. |
| `workflow_slug_reserved` | 409 | Create reused an explicit slug still held by a soft-deleted workflow. `errors` path is `slug`. |
| `workflow_deleted` | 409 | Resume, requeue, or claim found the workflow tombstoned. The run is `failed` with reason `workflow_deleted` and does not continue. Delete of a parked run uses the same terminal state in the delete transaction: steps and jobs that were `waiting`, `pending`, or `blocked` are `canceled` with error code `workflow_deleted`, and pending approvals are `canceled` with `closeReason` `workflow_deleted`. Step retry of that run is `execution_not_retryable` with reason `workflow_deleted`. |
| `approval_closed` | 409 | Decide on an approval that is `canceled` or `expired`, whose run is canceled or failed, whose workflow was deleted, or whose gate is no longer `waiting`. No decision is recorded as approved. A still-pending row whose gate has left `waiting` is closed as `expired` in that transaction. |
| `approval_requirement_unavailable` | 503 | Decide could not rebuild the pinned requirement because of a database, network, or timeout failure. `Retry-After` is `5`. Nothing is recorded and the approval is unchanged. The problem document has no `errors` array. |
| `requirement_unresolvable` | run `failed` | Not an HTTP problem code. A pending approval whose pinned requirement cannot be rebuilt is `canceled` with `closeReason` `requirement_unresolvable` and no decider. The waiting gate step and its job fail with that error code. The run follows the normal failed-step roll-up: a sibling already queued, claimed, or running finishes, then the run ends `failed` with that reason, which frees the concurrency slot. Downstream steps do not run, including a step wired to `expired`. This code means the pinned version is not found, policy evaluation failed deterministically, or the matching requirement is missing. A transient lookup is not this code. Compose claim of an unresolvable gate fails the job the same way, so lease recovery does not resume `expired`. A missing or deleted workflow still fails the run with `workflow_deleted`. |
