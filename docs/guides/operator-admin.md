...
| Rewrite surfaces (→ charter §11.1) | [rewrite-ui-surfaces.md](../reference/rewrite-ui-surfaces.md) — docs-only; not this walkthrough |
...
3. Use **left nav** — fail-closed from `GET /workspace`. Authoring:
 ...
 grant-gated and are **off product chrome** (R7.2 **keep
 open**); Settings may link carefully. Portal host is the E11.3
...
Rewrite surface map and operator migration notes (docs only;
charter §11.1):
...
1. Select **Unfiled** or a folder in the Explorer folder tree (`?folder=` deep
 ...
 no longer the primary organizer (F.2).
 Editors **New folder**, **Rename folder**, or **Delete folder**
 from the grant-gated right-click menu on the rail (F.3; X.2; X.8) — not as visible tree buttons. **New folder**
 creates the folder, then names it inline in the tree (X.7) — not a modal prompt.
 The folder tree uses compact Windows Explorer nav chrome (X.8): yellow folders,
 ...
 (F.5). Default search is
 **across folders** by name/slug; optional **in this
 folder** (F.6). The same
 rail + list + empty states + move land on
 `/embed/v1/workflows` after `session.embed` (F.7). The same Overview cards
 + compact Finder rail mount there (O.4). Missing `session.embed`
...
2. Toggle **List** / **Cards** (`aria-pressed`). The **Activation**
 column/status (R6.2) shows whether a
...
3. Create, import YAML, duplicate, and templates all `POST` a **draft**.
 ...
 version.
4. Drafts never execute. **Test run** (R6.3)
...
it back on Save draft (R2.5). Walkthrough:
...
3. **Add action** opens the wizard. **Undo (Ctrl+Z)** / **Redo
 ...
 before save. **Library**, **YAML**, **Inspector**, and **Runs** toggle satellites
...
7. **Test run** (R6.3) is one gesture:
...
8. **Activation** (R6.1) shows whether a
 **published** version is active (at least one enabled webhook or
 schedule pin). + R6 confirmation: D2 compose of enable
 ...
 is; one-gesture test-run is. Live / not-live stays
 icon + text (R7.4).
...
drops to the 16px grid (R2.4). Delete
removes the selection. Node positions persist as optional
`metadata.ui.layout` on draft save/load (R2.5 **keep
...
Webhooks / Schedules drawers (R7.4).
...
shell..1. **Node** — conversation chrome for this step: type-specific
 parameters / `with` (cataloged core / Kubernetes / SSH / script /
 HTTP — not a bare JSON blob; R3.1) /
 typed field-path mapping between ports (`allowedWith` / catalog
 port types; R3.2) / pins / credentials by
...
1. **Runs** is a remembered-open satellite scoped to this workflow
 ...
 and restores focus to **Runs** (R7.4).
 Status stays icon + text. After **Start published** or **Test run**
 the editor ends on this overlay for that run (UXL.4), not only a toast.
...
**not** product chrome (R7.2). Nav, search,
...
1. Workbench find (R5.1): **metadata only**. Search by **display name**
 ...
 stay on this page and are not sent to `GET /credentials`.
 R5 security line (inherit R5.2 /, R5.3): no KEK in
...
[embed-ui-evidence.md](../reference/e12-security-evidence/embed-ui-evidence.md).
