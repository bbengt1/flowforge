...
3. Use **left nav** — fail-closed from `GET /workspace`. Authoring:
 ...
 Foundation: Settings always. Membership / Isolation stay ADV-024
   grant-gated and are **off product chrome** (R7.2).
   Settings may link carefully. Portal host is the E11.3
...
1. Select **Unfiled** or a folder in the Explorer folder tree (`?folder=` deep
 ...
 from the grant-gated right-click menu on the rail (F.3 X.2 X.8 /
) — not as visible tree buttons. **New folder**
...
). The same Overview cards
 + compact Finder rail mount there (O.4 /
). Missing `session.embed`
...
`metadata.ui.layout` on draft save/load (R2.5).
Missing or invalid layout uses auto-layout. Invalid YAML
...
1. **Runs** is a remembered-open satellite scoped to this workflow
 ...
 the editor ends on this overlay for that run (UXL.4 /
), not only a toast.
