...
Deployable application skeleton (E1.1). Product design and contracts live in [docs/](docs/index.md) — start with [architecture](docs/architecture.md), the [Implemented vs Specified](docs/architecture/implemented-vs-specified.md) matrix, and the [MVP scope](docs/master-implementation-plan.md). Planning notes (gap analysis and the issue-creation backlog) live in [docs/internal/](docs/internal/README.md) and are not product docs.
...
4. Verify [http://localhost:8080/api/v1/health](http://localhost:8080/api/v1/health) then [http://localhost:8080/api/v1/readiness](http://localhost:8080/api/v1/readiness).
...
`/membership` exercises workspace identity and RBAC against the API contract. Next.js `/api/control-plane/*` proxies attach `X-FlowForge-*` identity headers and `X-Request-ID`. The operator never treats a host-supplied workspace UUID as the lookup key.
...
| `apps/api` | Go control plane. Compose `api` build context, `HTTP_ADDR=:8080` |
| `docs/` | Product architecture, MVP scope, and [Implemented vs Specified](docs/architecture/implemented-vs-specified.md) |
| `docs/internal/` | Planning and gap notes, not product docs. See [docs/internal/README.md](docs/internal/README.md) |
