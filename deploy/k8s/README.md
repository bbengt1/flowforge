...
The API process also applies forward-only migrations on connect. Production TLS is terminated at Ingress; the API ConfigMap sets `REQUIRE_TLS=true` and `TRUSTED_PROXY_CIDRS` for the cluster ranges.
