...
# Sibling of restore-rehearsal.sh (FFB1 AEAD dump + hardened API boot).
...
try:
    aead = str(root / "scripts/backup/aead.py")
    ...
    print(f"ok schema restore rehearsal version={got} isolated={restore_db} format=FFB1")
    evidence = {
